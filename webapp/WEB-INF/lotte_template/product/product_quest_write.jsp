...
<head>
    ...
    <% if (LOTTE_DEVEL_MODE) { %>
	    ...
	    <!-- 이 화면에서 쓰이는 CSS -->
	    <link rel="stylesheet" type="text/css" href="/lotte/resources_dev/product/product_quest_write.css"/>
	    <!-- 이 화면에서 쓰이는 JS -->
	    <script src="/lotte/resources_dev/product/product_quest_write.js"></script>
    <% } else {  %>
	    <%@include file="/common3/html_meta_angular.jsp"%>
	    <link rel="stylesheet" type="text/css" href="/lotte/resources/<%= pubVersion %>/product_quest_write.min.css"/>
	    <script src="/lotte/resources/<%= pubVersion %>/product_quest_write.min.js"></script>
	    <script src="/lotte/resources/<%= pubVersion %>/product_quest_write.tpl.js"></script>
    <% } %>
</head>
<body ng-controller="productQuestWriteCtrl">
