...
<head>
    <meta charset="utf-8">
    <title>롯데닷컴-new_prod_list</title>
    ...
    <%@include file="/angular_common/lotte_common_js.jsp"%>
     ...
     <% if(("Y").equals(request.getParameter("isWine"))) { %>
		<link rel="stylesheet" type="text/css" href="/lotte/resources_dev/brandshop/brand_common.css">
		<link rel="stylesheet" type="text/css" href="/lotte/resources_dev/specialHall/specialsHall.css">
     <% } else { %>
	    <!-- 이 화면에서 쓰이는 CSS -->
	    <link rel="stylesheet" type="text/css" href="/lotte/resources_dev/mall/gucci/gucci_main.css">
	    <!-- 이 화면에서 쓰이는 JS -->
     <% } %>
	    ...
	    <script src="/lotte/resources_dev/category/m/prod_list.js"></script>
    <% } else {  %>
	    <%@include file="/common3/html_meta_angular.jsp"%>
	    <% if(("Y").equals(request.getParameter("isWine"))) { %>
		    <link rel="stylesheet" type="text/css" href="/lotte/resources/<%= pubVersion %>/prod_list2.min.css"/>
		    <script src="/lotte/resources/<%= pubVersion %>/prod_list2.min.js"></script>
		    <script src="/lotte/resources/<%= pubVersion %>/prod_list2.tpl.js"></script>
	    <% } else { %>
		    <link rel="stylesheet" type="text/css" href="/lotte/resources/<%= pubVersion %>/prod_list.min.css"/>
		    <script src="/lotte/resources/<%= pubVersion %>/prod_list.min.js"></script>
		    <script src="/lotte/resources/<%= pubVersion %>/prod_list.tpl.js"></script>
	    <% } %>
...
<body ng-controller="ProdListCtrl">
