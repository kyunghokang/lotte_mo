...
<head>
    ...
    <% if (LOTTE_DEVEL_MODE) { %>
	    ...
	    <!-- 이 화면에서 쓰이는 CSS -->
        <link rel="stylesheet" type="text/css" href="/lotte/resources_dev/mylotte/product/m/mylotte_reinquiry_detail.css">
    	<!-- 이 화면에서 쓰이는 JS -->
        <script src="/lotte/resources_dev/mylotte/product/m/mylotte_reinquiry_detail.js"></script>
    <% } else {  %>
	    <%@include file="/common3/html_meta_angular.jsp"%>
	    <link rel="stylesheet" type="text/css" href="/lotte/resources/<%= pubVersion %>/mylotte_reinquiry_detail.min.css"/>
	    <script src="/lotte/resources/<%= pubVersion %>/mylotte_reinquiry_detail.min.js"></script>
	    <script src="/lotte/resources/<%= pubVersion %>/mylotte_reinquiry_detail.tpl.js"></script>
    <% } %>
</head>
<body ng-controller="mylotteReinquiryDetailCtrl" style="background-color:#f2f2f2">
