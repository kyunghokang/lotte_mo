...
<head>
    ...
    <% if (LOTTE_DEVEL_MODE) { %>
    	...
    	<!-- 이 화면에서 쓰이는 CSS -->
    	<link rel="stylesheet" type="text/css" href="/lotte/resources_dev/custcenter/m/question.css"/>
    	<!-- 이 화면에서 쓰이는 JS -->
    	<script src="/lotte/resources_dev/custcenter/m/question.js"></script>
    <% } else {  %>
	    <%@include file="/common3/html_meta_angular.jsp"%>
	    <link rel="stylesheet" type="text/css" href="/lotte/resources/<%= pubVersion %>/question.min.css"/>
	    <script src="/lotte/resources/<%= pubVersion %>/question.min.js"></script>
	    <script src="/lotte/resources/<%= pubVersion %>/question.tpl.js"></script>
    <% } %>
</head>
<body ng-controller="questionCtrl" style="background-color:#f2f2f2">
     <div id="wrapper" ng-show="showWrap">
     	<write-box ng-if="write_box || order_select_end"></write-box>
		<order-pop ng-if="order_pop" ></order-pop>	
