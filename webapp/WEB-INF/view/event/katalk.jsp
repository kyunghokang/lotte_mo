...
<head>
	...
	<% if (LOTTE_DEVEL_MODE) { // LOTTE_DEVEL_MODE %>
	
	<!-- common css -->
	<%@include file="/common3/html_meta_angular.jsp"%>
	 
	<!-- 이 화면에서만 쓰이는 CSS -->
	<link rel="stylesheet" href="<%=resourcesRoot%>/resources_dev/event/katalk.css"/>
	<!--// 이 화면에서만 쓰이는 CSS -->
	
	<!-- 이 화면에서 쓰이는 JS -->
	<script src="<%=resourcesRoot%>/resources_dev/event/katalk.js"></script>
	<script src="/common3/js/tablets.js"></script>

	<% } else {  %>
		<%@include file="/common3/html_meta_angular.jsp"%>
		<link rel="stylesheet" href="<%=resourcesRoot%>/resources/<%= pubVersion %>/katalk.min.css"/>
		<script src="<%=resourcesRoot%>/resources/<%= pubVersion %>/katalk.min.js"></script>
		<script src="/common3/js/tablets.js"></script>
	<% } %>
...
<body ng-controller="katalkCtrl">
	<div id="wrapper" ng-show="showWrap">
		...
		<%@ include file="/WEB-INF/view/event/katalk_for_angular.jsp"%>
