package com.lotte.mobile.common.servlet;
/**
 * Copyright MITRE
 *
 ...
 */
// original source : https://github.com/mitre/HTTP-Proxy-Servlet/blob/master/src/main/java/org/mitre/dsmiley/httpproxy/ProxyServlet.java
 ...
 * </p>
 * 
 * 사용목적 : mapi.lotte.com을 외부로 노출시키지 않고 m.lotte.com 내부 호출로 전환할 목적에 국한한 proxy servlet
 * 주요 수정 사항 : mapi.lotte.com이 response header에 set-cookie를 통해 쿠키를 설정하려 할 경우
 * .lotte.com(HOST_COOKIE_DOMAIN)인 경우에는 .lotte.com 그대로 set-cookie
 * mapi.lotte.com인 경우에는 m.lotte.com(HOST_MOBILE)로 set-cookie를 실행시키도록 한다. 
 *
...
/* INIT PARAMETER NAME CONSTANTS */
  ...
  protected static final String XSRF = "XSRF-TOKEN";
  ...
  // lotte cookie domain
  protected String lotteCookieDomain = null;
  // ellotte cookie domain
  protected String ellotteCookieDomain = null;
  // m.lotte.com
  protected String mobileCookieDomain = null;
  ...
  protected void initTarget() throws ServletException {
    ...
    if (targetUri == null) {
    	targetUri = MLotteProperties.get("HOST_API")+"/json";
    }
    
    this.lotteCookieDomain = MLotteProperties.get("HOST_COOKIE_DOMAIN");
    this.ellotteCookieDomain = MLotteProperties.get("ELLOTTE_HOST_COOKIE_DOMAIN");
    String get_host_mobile = MLotteProperties.get("HOST_MOBILE");
    get_host_mobile.substring("http://".length(), get_host_mobile.length());
    this.mobileCookieDomain = get_host_mobile;
  ...
  protected void copyProxyCookie(HttpServletRequest servletRequest,
    ...
    for (HttpCookie cookie : cookies) {
    	...
    	if (XSRF.equals(cookieName)) {
    		continue;
    	}
    	if (JSESSIONID.equals(cookieName)) {
    		cookieName = getCookieNamePrefix() + cookie.getName();
    		path = servletRequest.getContextPath()+servletRequest.getServletPath();
    	}
    	...
    	if (lotteCookieDomain.equals(cookie.getDomain())) {
    		servletCookie.setDomain(lotteCookieDomain);
    	} else if (ellotteCookieDomain.equals(cookie.getDomain())) {
    		servletCookie.setDomain(ellotteCookieDomain);
    	}
  ...
  protected String getRealCookie(String cookieValue) {
    ...
    for (String cookie : cookies) {
    	if (cookie.startsWith(JSESSIONID+"=")) { // m.lotte.com의 jssessionid는 mapi.lotte.com으로 복사하지 않는다.
    		continue;
    	}
    	if (!cookie.startsWith(getCookieNamePrefix())) {
    		escapedCookie.append(cookie+"; ");
    		continue;
    	}
    	cookie = cookie.substring(getCookieNamePrefix().length(), cookie.length());
    	escapedCookie.append(cookie+"; ");
    }
    
    if (escapedCookie.toString().endsWith("; ")) {
    	escapedCookie.delete(escapedCookie.length()-"; ".length(), escapedCookie.length());
    }
