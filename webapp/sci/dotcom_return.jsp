...
// 20170922 박형윤 iOS 본인인증 후 성인여부 확인 값 스키마 전달 추가
// NativeApp 세분화
var nativeAppInfo = {
    isApp: false,
    isSKT: false,
...
/**
 * 본인인증 후 성인여부값 앱 스키마 전달
 * @param {string} Y/N
 */
function changeAdultYN(adultFlag) {
	if (nativeAppInfo.isApp) {
		try {
			if (nativeAppInfo.isIOS && nativeAppInfo.versionNum > 2840) { // iOS
				location.href = 'lottebridge://adultChk/?' + adultFlag;
			} else if (nativeAppInfo.isAndroid && nativeAppInfo.versionNum > 295) { // Android
				window.lottebridge.adultChk(adultFlag);
			}
		} catch (e) {}
	}
}
