(function(window, angular, undefined) {
		...
		app.controller('wishCtrl', ['$scope', '$http', '$filter', 'LotteCommon', 'LotteCookie', 'LotteUtil', '$timeout', '$window', 'LotteStorage', 'commInitData', 'lotteNgSwipe', 'LotteUserService', 
			function($scope, $http, $filter, LotteCommon, LotteCookie, LotteUtil, $timeout, $window, LotteStorage, commInitData, lotteNgSwipe, LotteUserService) {
			//----------------------------------------------------------------------------------------------------
				// 변수 선언
				//----------------------------------------------------------------------------------------------------
			...
			if(!$scope.appObj.isApp || (($scope.appObj.isAndroid && $scope.appObj.verNumber >= 400) || ($scope.appObj.isIOS && $scope.appObj.verNumber >= 4000))){ // 앱 4.0 이상 보이게 처리
				$scope.categoryList = [
					[
						{"ctg_nm":"전체보기",		"curDispNo":""},
						{"ctg_nm":"오늘 장보기",			"curDispNo":"lotteSuper", "isSuperCate": true},
			...
			$scope.existMoreItem = function() {
				...
				if ($scope.currentCategory == "" || $scope.currentCategory == "lotteSuper") { // '전체보기', '장보기몰'
					rtnBoolean = $scope.loadedProductList.length + $scope.superWishListInfo.showWishList.length < $scope.productTotalCount + $scope.superWishListInfo.totalCnt;
				} else {
					rtnBoolean = $scope.loadedProductList.length < $scope.productTotalCount;
				}
			...
			$scope.notExistWishItem = function() {
				...
				if ($scope.currentCategory == "lotteSuper") { // '장보기몰' 카테고리
					rtnBoolean = false;
				} else if ($scope.currentCategory == "") { // '전체' 카테고리
					rtnBoolean = $scope.initLoadingProductList && $scope.loadedProductList.length == 0 && $scope.superWishListInfo.showWishList.length == 0;
				} else {
					rtnBoolean = $scope.initLoadingProductList && $scope.loadedProductList.length == 0;
				}
...
//        		}
			...
			$scope.deleteAllProduct = function() {
				...
				if ($scope.currentCategory != "lotteSuper") { // '장보기몰' 카테고리시에는 닷컴 위시 동작 안하도록
					// 일반상품 전체 삭제
				...
				// 수퍼상품 삭제 추가
				if (($scope.superWishListInfo.showWishList && $scope.superWishListInfo.showWishList.length > 0) || $scope.superWishListInfo.wishGoodsList && $scope.superWishListInfo.wishGoodsList.length > 0) {
					var i = 0,
						targetSeqArr = [];

					// 보여지는 상품들만 삭제
					for (i = 0; i < $scope.superWishListInfo.showWishList.length; i++) {
						targetSeqArr.push($scope.superWishListInfo.showWishList[i].seq);
					}

					// 안보여지는 상품들도 삭제
					for (i = 0; i < $scope.superWishListInfo.wishGoodsList.length; i++) {
						targetSeqArr.push($scope.superWishListInfo.wishGoodsList[i].seq);
					}

					$scope.delMultiSuperWish(targetSeqArr);
				}
			}
			...
			$scope.deleteProduct = function(itemSeqs, tclickFlag) {
				...
				if (0 == $scope.getCheckedProducts().length && 0 == $scope.getCheckedSuperProdGoodsNo().length) {
				...
				// 롯데슈퍼
				if (0 < $scope.getCheckedSuperProdGoodsNo().length) {
					// 수퍼상품 삭제 추가
					$scope.delMultiSuperWish($scope.getCheckedSuperProdGoodsNo());
				}
			};
			...
			$scope.addCart2 = function(index, successCallback, errorCallback) {
				...
				.success(function(data) {
					...
					$scope.alidoCollectLog('wishCartAdd', product); // ALIDO 수집스크립트 호출
				})
				.error(function(ex) {
					ajaxResponseErrorHandler(ex, function() {
						if (errorCallback) errorCallback(index);
...
//        			goAdultSci();
			...
			$scope.wishCateClick = function(cate) {
				...
				// 롯데 슈퍼 위시리스트 초기화
				Array.prototype.push.apply($scope.superWishListInfo.showWishList, $scope.superWishListInfo.wishGoodsList);
				$scope.superWishListInfo.wishGoodsList = $scope.superWishListInfo.showWishList;
				$scope.superWishListInfo.showWishList = [];

				if ($scope.currentCategory == "lotteSuper") {
					$scope.loadedProductList = [];
					$scope.productTotalCount = 0;

					$scope.showSuperWishList();
				} else {
			...
			$scope.loadMoreProduct = function() {
				...
				if ($scope.currentPageNo != 0 && $scope.loadedProductList.length == $scope.productTotalCount) {
					$scope.currentPageNo++;
				...
				// TODO ywkang2 : 로딩 기준 가이드 적용
				$http.get($scope.makeProductListUrl()) // 여기에서 현재 페이지 번호 1 증가 됨
				.success(function(data) {
					if (data.result) {
					...
					// 슈퍼 위시리스트 API 호출
					if (!$scope.superWishListInfo.isLoaded) {
						if(!$scope.appObj.isApp || (($scope.appObj.isAndroid && $scope.appObj.verNumber >= 400) || ($scope.appObj.isIOS && $scope.appObj.verNumber >= 4000))){ // 앱 4.0 이상 보이게 처리
							$scope.getLotteSuperWish(); // 위시리스트 접근 시 슈퍼쪽 위시리스트도 호출
						}
					} else {
						$scope.showSuperWishList(); // 슈퍼 위시 리스트 노출 판단
					}
			...
			//----------------------------------------------------------------------------------------------------
			// 롯데 슈퍼 추가 20180108 박형윤
			//----------------------------------------------------------------------------------------------------
			// 장보기몰 데이터가 없을경우 링크
			$scope.lsMainClick = function() {
				var lsMainCHLNO = ($scope.appObj.isSuperApp) ? "M389213" : "M389193"; // 슈퍼 base : 닷컴 base
				if(!$scope.appObj.isApp){ lsMainCHLNO = "M389475"; }
				$scope.gotoLotteSuperUrl(LotteCommon.SuperMainUrl, lsMainCHLNO); // param (url, 채널)
			};
			
			// 닷컴/슈퍼 롯데슈퍼 위시 관련 데이터 20180110
			$scope.superWishListInfo = {
				isLoaded: false,
				totalCnt: 0,
				showWishList: [],
				wishGoodsList: [],
			...
			// 닷컴/슈퍼 폴더앱 추가 20180108 박형윤
			$scope.getLotteSuperWish = function () {
				$scope.superWishListInfo.isLoaded = true;

				var paramsObj = {};

				$scope.getDeferLotteSuperPostParam(paramsObj).then(function (postParam) {
					var superWishListApiUrl = LotteCommon.SuperWishListData;
					var method = "post";
	
					// 로컬 테스트
					if (window.location.host.indexOf("localhost") > -1) {
						superWishListApiUrl = "/json/lottesuper/wish_list.json";
						method = "get";
					}
	
					$.ajax({
						url: superWishListApiUrl,
						method: method,
						data: postParam
					}).success(function (data) {

						$scope.superWishListInfo.wishGoodsList = [];

						if (data.resultCode && data.resultCode == "1" && 
							data.goodsList && data.goodsList.length > 0) {

							angular.forEach(data.goodsList, function (value, key) {
								if (value.ADULT_GOODS_YN == "N" && // 성인상품 제외
									value.STOCKOUT == "N" // 품절상품 제외
								) {
									value.seq = key; // 위시 삭제일 경우 sequence를 할당하여 지워주기 위해 강제 주입
									$scope.superWishListInfo.wishGoodsList.push(value);
								}
							});

							// $scope.superWishListInfo.wishGoodsList = data.goodsList;
							$scope.superWishListInfo.totalCnt = $scope.superWishListInfo.wishGoodsList.length;

							$scope.showSuperWishList(); // 슈퍼 위시 리스트 노출 판단
						} else if (data.dotComAcceptCode == "-999") { // 슈퍼 강제 연동 제외
							$scope.superWishListInfo.systemError = true;
						}
					}).error(function (res) {
			 ...
			 * 체크된 슈퍼 상품 번호 구하기
			 */
			$scope.getCheckedSuperProdGoodsNo = function() {
				var result = [];
				var checkedItem = $filter('filter')($scope.superWishListInfo.showWishList, {is_checked: true});

				angular.forEach(checkedItem, function(product, idx) {
					result.push(product.seq);
				});
			...
			// 슈퍼 위시리스트 단건 삭제하기
			$scope.delSuperWish = function (seq) {
				$scope.sendTclick("m_DC_wish_Clk_Btn_7");
				if (!confirm("상품을 삭제하시겠습니까?")) {
					return false;
				}

				$scope.delMultiSuperWish([seq]);
			};

			// 슈퍼 위시리스트 다중 삭제하기
			$scope.delMultiSuperWish = function (seqArr) {
				if (seqArr.length == 0) {
					return false;
				}
				...
				// 보여지는 상품
				for (i = 0; i < $scope.superWishListInfo.showWishList.length; i++) {
					if (seqArr.indexOf($scope.superWishListInfo.showWishList[i].seq) > -1) {
						targetGoodsNoArr.push($scope.superWishListInfo.showWishList[i].GOODS_NO);
					}
				}

				// 안보여지는 상품
				for (i = 0; i < $scope.superWishListInfo.wishGoodsList.length; i++) {
					if (seqArr.indexOf($scope.superWishListInfo.wishGoodsList[i].seq) > -1) {
						targetGoodsNoArr.push($scope.superWishListInfo.wishGoodsList[i].GOODS_NO);
					}
				}

				var paramsObj = {};
				paramsObj.checkIdx = targetGoodsNoArr.join(","); // 삭제항목 추가

				$scope.getDeferLotteSuperPostParam(paramsObj).then(function (postParam) {
					var superDelWishApiUrl = LotteCommon.SuperWishDeleteData;
					var method = "post";
	
					// 로컬 테스트
					if (window.location.host.indexOf("localhost") > -1) {
						superDelWishApiUrl = "/json/lottesuper/cart_list.json";
						method = "get";
					}

					$.ajax({
						url: superDelWishApiUrl,
						method: method,
						data: postParam
					}).success(function (data) {
						if (data.resultCode == "1") {
							...
							// 전체 개수에서 마이너스 해줄 것
							var i = $scope.superWishListInfo.showWishList.length - 1;
	
							// 보여지는 상품
							for (i = $scope.superWishListInfo.showWishList.length - 1; i >= 0; i--) {
								if (seqArr.indexOf($scope.superWishListInfo.showWishList[i].seq) > -1) {
									$scope.superWishListInfo.showWishList.splice(i, 1);
								}
							}
	
							// 안보여지는 상품
							for (i = $scope.superWishListInfo.wishGoodsList.length - 1; i >= 0; i--) {
								if (seqArr.indexOf($scope.superWishListInfo.wishGoodsList[i].seq) > -1) {
									$scope.superWishListInfo.wishGoodsList.splice(i, 1);
								}
							}
	
							$scope.superWishListInfo.totalCnt -= seqArr.length;
						} else {
							alert("롯데슈퍼 위시리스트 삭제에 실패하였습니다. (" + decodeURIComponent(data.resultMsg) + ")");
						}
					}).error(function (res) {
						console.log("error", res);
					});
			...
			/*
			 * 슈퍼 위시리스트 장바구니 담기 / 바로구매
			 * @params
			 * prod : 상품 정보
			 * drtPurYn : 바로구매여부 (Y/N)
			 */
			$scope.superWishCartOrder = function (prod, drtPurYn) {
				
				if(drtPurYn == "Y"){ // 바로구매
					$scope.sendTclick("m_DC_wish_Clk_Btn_9");
				} else if(drtPurYn == "N"){ // 장바구니
					$scope.sendTclick("m_DC_wish_Clk_Btn_8");
				}
				
				if (prod.OPT_GOODS_SCT_CD != 10) { // 10이 아니면 옵션 있는 상품
					alert("이 상품은 옵션이 있는 상품입니다. 상품상세에서 옵션을 선택해 주세요.");
					$scope.goSuperProd(prod.GOODS_NO);
				...
				paramsObj.goods_no = prod.GOODS_NO, // 상품번호
				paramsObj.ord_qty = prod.ORD_PSB_MIN_QTY, // 주문 최소 수량
				paramsObj.sale_shop_sct_cd = "21", // 매출구분코드 (21 고정)
				paramsObj.disp_cat_no = prod.DISP_CAT_NO, // 전시카테고리번호
				paramsObj.drt_pur_yn = drtPurYn // 바로구매여부 ("N": 장바구니담기, "Y": 바로구매)

				$scope.getDeferLotteSuperPostParam(paramsObj).then(function (postParam) {
					var superInsertCartApiUrl = LotteCommon.SuperWishInsertCartData;
					var method = "post";

					// 로컬 테스트
					if (window.location.host.indexOf("localhost") > -1) {
						superInsertCartApiUrl = "/json/lottesuper/wish_list_cart.json";
						method = "get";
					}
	
					$.ajax({
						url: superInsertCartApiUrl,
						method: method,
						data: postParam
					}).success(function (data) {
						if (data.resultCode == "1") { // 장바구니 담기 성공
	
							if (postParam.drt_pur_yn == "Y") { // 바로구매일때
								var superWishCHLNO = ($scope.appObj.isSuperApp) ? "M389212" : "M389192"; // 슈퍼 base : 닷컴 base
								if(!$scope.appObj.isApp){ superWishCHLNO = "M389474"; }
	
								$scope.gotoLotteSuperBuyUrl(LotteCommon.SuperDirectBuyUrl + "?drt_pur_yn=Y&cart_no_dotcom_if=" + data.cart_no_dotcom_if, superWishCHLNO); // param (url, 채널)
							...
							if (data.dotComAcceptCode == "-999") { // 슈퍼 강제 연동 제외
								$scope.superWishListInfo.systemError = true;
							}
	
							if (postParam.drt_pur_yn == "Y") { // 바로구매일때
								alert("바로구매에 실패하였습니다. (" + decodeURIComponent(data.resultMsg) + ")");
							} else { // 장바구니 담기 일때
								alert("롯데슈퍼 장바구니 담기에 실패하였습니다. (" + decodeURIComponent(data.resultMsg) + ")");
							}
						}
					}).error(function (res) {
						console.log("error", res);
			...
			// 슈퍼 상품상세 이동
			$scope.goSuperProd = function (goodsNo) {
				 var superWishCHLNO = ($scope.appObj.isSuperApp) ? "M389212" : "M389192"; // 슈퍼 base : 닷컴 base
				 if(!$scope.appObj.isApp){ superWishCHLNO = "M389474"; }
				$scope.gotoLotteSuperUrl(LotteCommon.SuperProdDetailUrl + "?goods_no=" + goodsNo, superWishCHLNO); // param (url, 채널)
			};

			// 슈퍼 위시리스트 체크박스 체크하기
			$scope.chkSuperWish = function (idx) {
				if ($scope.superWishListInfo.showWishList[idx]) {
					$scope.superWishListInfo.showWishList[idx].is_checked = !$scope.superWishListInfo.showWishList[idx].is_checked;
				}
			};

			// 슈퍼 위시리스트 노출 개수 조정 (슈퍼 위시 리스트 노출 판단)
			$scope.showSuperWishList = function () {
				/*
				슈퍼 위시리스트 정보
				$scope.superWishListInfo = {
					isLoaded: false,
				...
				*/
				// 닷컴 위시리스트 전체 개수 : $scope.productTotalCount
				// 닷컴 위시리스트 로드 개수 : $scope.loadedProductList
				// 페이지당 노출 개수 : $scope.itemCountPerPage
				// 현재 로드 페이지 번호 : $scope.currentPageNo (1페이지 부터 시작)
				// 현재 선택 카테고리 번호 : $scope.currentCategory
				if ($scope.currentCategory == "" || $scope.currentCategory == "lotteSuper") { // 전체보기, 장보기몰에만 슈퍼 위시 노출
					var showCnt = 0;
					
					if ($scope.itemCountPerPage - (($scope.loadedProductList.length + $scope.superWishListInfo.showWishList.length) % $scope.itemCountPerPage) > 0 || $scope.currentCategory == "lotteSuper") {
						showCnt = $scope.itemCountPerPage - (($scope.loadedProductList.length + $scope.superWishListInfo.showWishList.length) % $scope.itemCountPerPage);
					} else if ($scope.loadedProductList.length == 0) {
						showCnt = $scope.itemCountPerPage;
					}

					Array.prototype.push.apply($scope.superWishListInfo.showWishList, $scope.superWishListInfo.wishGoodsList.splice(0, showCnt));
				} else {
					$scope.superWishListInfo.showWishList = [];
				}
			};
			 ...
			 * 슈퍼 한글 텍스트 Decode
			 */
			$scope.getDecodeUri = function(str) {
				str = decodeURIComponent(str).replace(/\+/gi, " ");
			 ...
			 * 수퍼 할인가 계산
			 */
			$scope.getSuperSaleRate = function (prd) {
				var saleRate = 0;

				if (prd.NORM_PRC && prd.SALE_PRC && prd.NORM_PRC > prd.SALE_PRC) {
					saleRate = (prd.NORM_PRC - prd.SALE_PRC) / prd.NORM_PRC * 100;
				}

				return parseInt(saleRate);
			};
			...
			angular.element($window).on('unload', function() {
				...
				LotteStorage.setSessionStorage(
					$scope.screenId + 'Data',
					{
							...
							superWishListInfo: $scope.superWishListInfo
					},
					'json'
				);
			});
			...
			if (window.location.hash == ('#/' + $scope.historyBackFlag)) {
				...
				// 롯데슈퍼 위시리스트
				$scope.superWishListInfo = storedData.superWishListInfo;
		...
		app.directive('lotteContainer', function() {
			return {
				templateUrl: '/lotte/resources_dev/mylotte/wish/m/wish_container_2018.html',
				replace: true,
		...
		app.directive('wishlistNormalProductList', function() {
			return {
				restrict: 'E',
				templateUrl: '/lotte/resources_dev/mylotte/wish/m/wish_normal_list_container_2018.html',
				replace: true,
				link: function($scope, el, attrs) {}
			};
		});
