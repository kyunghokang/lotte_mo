...
(function(window, angular, undefined) {
	 ...
	 * 장바구니 메인 컨트롤러
	 */
	app.controller('cartCtrl', ['$scope', 'LotteCommon', '$http', 'LotteCookie', '$filter', 'LotteUtil', '$timeout', '$window', function($scope, LotteCommon, $http, LotteCookie, $filter, LotteUtil, $timeout, $window) {
		//----------------------------------------------------------------------------------------------------
		// 변수 선언
		//----------------------------------------------------------------------------------------------------
			...
			if (!product.is_checked) {
				...
				angular.forEach(groupInfo.products, function(product, idx) {
					if (!product.is_checked) {
						...
						selPrd += product.quick_deli_yn==quick_yn ? '':'N';//선택되어있는 상품이 퀵배송상품인지 일반상품인지 체크
					}
				});
				if(selPrd.indexOf('N') > -1){
		 ...
		 */
		$scope.goGoodsDetail = function(product, isSoldout) {
			var wish_lst_sn = '';
			var item_no = '';
...
//            });
		...
		$scope.makeProductListUrl = function() {
			...
			productListUrlParams.push('viewGoods=' + localStorage.getItem('latelyGoods'));
			productListUrlParams.push('amlist=' + localStorage.getItem('amlist'));

			var url = '';

			if ($scope.useTestData) {
				url = '/lotte/resources_dev/data/mylotte/cart/m/cart_list_data.html';
			} else {
				url = LotteCommon.cartListData + '?' + $scope.baseParam + '&' + productListUrlParams.join('&');
			}
	...
	app.directive('lotteContainer', function() {
		return {
			templateUrl : '/lotte/resources_dev/mylotte/cart/m/cart_container.html',
			replace : true,
	...
	app.directive('cartGroup', ['$http', 'LotteCommon', 'LotteUtil','LotteCookie', 'LotteUserService', function($http, LotteCommon, LotteUtil, LotteCookie, LotteUserService) {
		return {
	...
	app.controller('CartGroupCtrl', ['$scope', '$http', 'LotteCommon', function ($scope, $http, LotteCommon) {
...
//          $scope.datePickerInputOption = {};
		...
		$scope.toggleCheckAllItemsFromGroup = function(e) {
			var isRentalPdt = false, quickPrdNum = 0;
			...
			if(!isRentalPdt) {
				if(quickPrdNum && $scope.groupInfo.products.length != quickPrdNum){
					alert('택배 배송 상품과 퀵 배송 상품은\n함께 주문하실 수 없습니다.');
...
//                  $scope.showInstantMessage('최소 ' + minimumOrderQty + '개 구매 가능한 상품입니다.');
		 ...
		 * 상품 상세 화면으로 이동
		 */
		$scope.goGoodsDetailFromGroup = function(product) {
			$scope.goGoodsDetail({
				product: product,
				isSoldout: false
			});
