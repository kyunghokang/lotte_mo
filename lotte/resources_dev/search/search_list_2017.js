(function(window, angular, undefined) {
    ...
    app.controller('SearchList',
    		['$scope', '$http', '$filter', '$location', '$window', '$timeout', 'LotteCommon', 'LotteUtil', 'LotteLink', 'commInitData', 'LotteScroll', 'LotteStorage', 'LotteUserService','$compile',
    		function($scope, $http, $filter, $location, $window, $timeout, LotteCommon, LotteUtil, LotteLink, commInitData, LotteScroll, LotteStorage, LotteUserService, $compile) {
		...
		//$scope.templateType1 = "search_list_2017";
		...
		$scope.uiStateObj = { /*UI 상태값 저장*/
			...
			srhFilterBrdSort : "brdName", /*검색필터 - 브랜드탭 Sortting 값*/
        ...
        */
        ...
        $scope.setSurveyPos = function(){
        	...
        	var pos = 29;
        	/*if($scope.templateType1 == "search_image6_2017"){
        		pos = 23;
        	}*/
        	var ell;
        ...
        $scope.checkDealListShow = function(){
        	/*if($scope.templateType1 == "search_image6_2017"){//큐레이션이면 비노출
        		return false;
        	}*/
        ...
        $scope.retriveTemplateType = function(){
        	...
        	/*if($scope.uiStateObj.emptyResult){
        		//$scope.templateType1 = "search_list_2017";
        		$scope.templateType2 = "search_list_2017";
        	}else{
				if($scope.uiStateObj.ageGenderSearched || $scope.postParams.isVoice == "Y" || $scope.postParams.isVoice == "T"){
					//$scope.templateType1 = "search_list_2017";
					$scope.templateType2 = "search_list_2017";
				}else{
					if($scope.srhResultData.dispUnitType != undefined && $scope.srhResultData.dispUnitType == "01" && $scope.srhResultData.tCnt > 6){
						// 큐레이션형
						//$scope.templateType1 = "search_image6_2017";
						$scope.templateType2 = "search_image_2017";
					}else{
						// 일반형
						//$scope.templateType1 = "search_list_2017";
		...
		$scope.loadListData = function (params) {
			...
			// 카테고리 선택시 초기화되는 조건 선택여부
			$scope.ctgInitFlag = false;
			// 최고값이 있고 최저값 입력 없을시 기본 최저 값으로 세팅
			if (postParams.priceMaxU && !postParams.priceMinU) {
				postParams.priceMinU = $scope.srhResultData.price.min;
				$scope.ctgInitFlag = true;
			}
			// 최저값이 있고 최고값 입력 없을시 기본 최저 값으로 세팅
			if (postParams.priceMinU && !postParams.priceMaxU) {
				postParams.priceMaxU = $scope.srhResultData.price.max;
			...
			if((postParams.brdNo) 
			   || (postParams.freeDeliYN && postParams.freeDeliYN == 'Y') 
			   || (postParams.freeInstYN && postParams.freeInstYN == 'Y') 
			   || (postParams.pointYN && postParams.pointYN == 'Y') 
			   || (postParams.pkgYN && postParams.pkgYN == 'Y') 
			   || (postParams.smpickYN && postParams.smpickYN == 'Y') 
			   || (postParams.delSevenYN && postParams.delSevenYN == 'Y') 
			   || (postParams.delTdarYN && postParams.delTdarYN == 'Y') 
			   || (postParams.delQuickYN && postParams.delQuickYN == 'Y') 
			   || (postParams.colorCd) 
			   || (postParams.deptYN && postParams.deptYN == 'Y') 
			   || (postParams.tvhomeYN && postParams.tvhomeYN == 'Y') 
			   || (postParams.superYN && postParams.superYN == 'Y') 
			   || (postParams.brdstoreYN && postParams.brdstoreYN == 'Y')){
				$scope.ctgInitFlag = true;
			}
		...
		function splitProductList(){
			...
			if(len <= 18){
				...
				$scope.srhResultData.listMid2 = $scope.srhResultData.prdLst.items.slice(0, 18);
				$scope.srhResultData.listRest = [];
			}else{
				//$scope.srhResultData.listTop6 = $scope.srhResultData.prdLst.items.slice(0, 6);
				$scope.srhResultData.listMid2 = $scope.srhResultData.prdLst.items.slice(0, 18);
    ...
    app.directive('lotteContainer', ['LotteStorage', 'LotteUtil', 'LotteCommon', '$http', '$timeout', '$window', '$filter', 
        ...
        return {
            templateUrl : '/lotte/resources_dev/search/search_list_2017_container.html',
			replace : true,
			link:function($scope, el, attrs) {
				var $win = angular.element($window);
				...
				$scope.showHideSortGuide = function(){
					...
					if($scope.tipShow){
						...
						$scope.searchTclick2016("Sorting_Tip");
					}
				};
				...
				$scope.srhSortPost = function (idx) {
					...
					switch (idx) {
						case 0 : $scope.searchTclick2016("Sorting_Best"); break; //인기순BEST
						case 1 : $scope.searchTclick2016("Sorting_Sell"); break; //판매BEST
						case 2 : $scope.searchTclick2016("Sorting_Review"); break; //상품평많은순
						case 3 : $scope.searchTclick2016("Sorting_Recent"); break; //최근등록순
						case 4 : $scope.searchTclick2016("Sorting_Lprice"); break; //낮은가격순
						case 5 : $scope.searchTclick2016("Sorting_Hprice"); break; //높은가격순
					}
					$win.scrollTop(0);
				};
				...
				$scope.srhDetailChange = function (type){
					...
					*/
					...
					var tClickStr = "";
					switch (type) {
						case "deptYN" 		: $scope.searchTclick2016("Store_01"); break; //롯데백화점
						case "tvhomeYN" 	: $scope.searchTclick2016("Store_02"); break; //롯데홈쇼핑
						case "superYN" 		: $scope.searchTclick2016("Store_03"); break; //롯데슈퍼
						case "brdstoreYN" 	: $scope.searchTclick2016("Store_04"); break; //브랜드스토어
						
						case "freeDeliYN"	: $scope.searchTclick2016("Clk_FreeShipping"); break; //무료배송
						case "smpickYN"		: $scope.searchTclick2016("benefit_smartpick"); break; //스마트픽
						case "freeInstYN"	: $scope.searchTclick2016("Price_Interest"); break; //무이자
						case "pointYN"		: $scope.searchTclick2016("Price_Point"); break; //포인트
						case "pkgYN"		: $scope.searchTclick2016("Price_Packing"); break; //무료포장
						
						case "delSevenYN"	: $scope.searchTclick2016("Shipping_Smartpick_Store"); break;//세븐일레븐
						case "delTdarYN"	: $scope.searchTclick2016("Shipping_Oneday"); break;//오늘배송
						case "delQuickYN"	: $scope.searchTclick2016("Shipping_Quick"); break;//퀵배송
					}
				...
				$scope.checkResetDisabled = function(flag){
					...
					if($scope.searchUISetting.isShowSub==false){
						...
						if(
							...
							if(flag != 'detail' && postParams.reQuery.length==0 && postParams.exQuery.length==0) {
								return "disabled";
							} else if(flag == 'detail') {
								return "disabled";
							}
						...
						if(
							...
							|| (searchUISetting.slide=='benefit' && postParams.freeDeliYN+postParams.freeInstYN+postParams.pointYN+postParams.pkgYN=='NNNN' && postParams.priceMaxU==null && postParams.priceMinU==null)
				...
				$scope.srhFilterCtgInit = function (reload) {
					...
					$(".ssws_wrap.ssws_category .cate_d1").height( 44 * $(".ssws_wrap.ssws_category .cate_d1 > li").length );
				...
				$scope.searchFromApp = function(data){
					...
					if(data.brdNo != undefined && data.brdNo != "" && $scope.srhResultData.brdLst && $scope.srhResultData.brdLst.items){
						...
						$scope.uiStateObj.brandArr.length = 0;
