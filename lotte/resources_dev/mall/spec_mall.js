...
(function(window, angular, undefined) {
	...
	app.controller('SpecMallCtrl', ['$http', '$scope', 'LotteCommon', 'commInitData', 'LotteUtil', '$location','$window','$timeout', function($http, $scope, LotteCommon, commInitData, LotteUtil, $location,$window, $timeout) {
		...
		//test data
		//$scope.testData = '/json/mall/getKshopMain_5617246.json';
        ...
        if(commInitData.query.dispNo == "5617246"){
            $scope.kShopUI.type = 'mk_template';
        }
        ...
        $scope.linkLotteGroup = function(url, index1, index2){
            var tclick = index1*3 + index2 + 1;
            if(tclick < 10){
                tclick = "0" + tclick;
            }
            tclick = "m_LOTTEG_Store_" + tclick;
            window.location.href = url + "&" + $scope.baseParam +"&tclick=" + tclick;
        }	
        
        $scope.clickLotteStoreMore = function(){
        	$scope.lotteStoreMore = true;
        };
		...
		$scope.loadMainData = function () {
			if ($scope.mainData == undefined) {
				...
				// var url = ($scope.testData != undefined && $scope.testData !='') ? $scope.testData : LotteCommon.specMall2017Main;
				var url = LotteCommon.specMall2017Main;
				...
				.success(function(json){
					...
					// 20180608 마이클코어스 메인데이터 분기 (구분값 = 매장번호 :5617246)
                    if(kshop.type){
                        $scope.kShopUI.type = kshop.type;
                    }
					if($scope.kShopUI.type == "mk_template"){                         
						$scope.loadMkData(kshop);
						$scope.mainData =  kshop;
					}else{
						$scope.mainData = data;
					}
                    setTimeout(function(){
                        angular.element(".smcCateContainer").scope().setCateType(); //카테고리 타입 세팅    
                    }, 100);
				...
				//console.log($scope.mainData);
                    ...
                    if($scope.mainData.lotteStore && $scope.mainData.lotteStore.length){
                        $scope.lotteStore = [];
                        var arrIndex = 0;
					...
					angular.element($window).on("resize.sbnr", function() {
						...
						// 20180608 mainData.prdList 유무 체크
						if($scope.mainData.prdList){
		 ...
		 * @name loadMkData
		 * 마이클코어스 메인데이터 재조합
		 * @param {data} object kshop 데이터
		*/
		$scope.loadMkData = function(data){
			...
			if(data.logo &&  data.logo != undefined ){
				$scope.MkMallmain.mkLogo = data.logo; 
			}
			
			//마이클코어스 탑배너
			if(data.mkTopBanner &&  data.mkTopBanner != undefined ){
				$scope.MkMallmain.mkTopBanner = data.mkTopBanner; 
			}	
			
			//마이클코어스 타이틀
			if(data.mkTitle &&  data.mkTitle != undefined ){
				$scope.MkMallmain.mkTitle = data.mkTitle; 
			}	
			...
			angular.forEach(data,function(val,key){
				for(var i =1;i<7;i++){
					...
					if(val && key == "mkInventory"+i){
						if(i < 5){
							$scope.MkMallmain.mkInventoryTop.push(val);
						}else{
							$scope.MkMallmain.mkInventoryBtm.push(val);
						}
					}
				}
			...
			// 인벤토리 하단 배너 영역 시작 index 세팅 (tclick 사용)
			$scope.startIdx = $scope.MkMallmain.mkInventoryTop.length ;

			// 마이클코어스 인벤토리(상품 배너) mkInventoryBtm 배열 첫번째 넣기
			if(data.mkPrdBanner && data.mkPrdBanner != undefined){
				data.mkPrdBanner.prdChk = true; // 상품배너 구분값

				$scope.startIdx =  $scope.startIdx -1 ; //상품 영역이 존재하면 상품 배너 넘버링 제외 (tclick 사용)

				$scope.MkMallmain.mkInventoryBtm.unshift(data.mkPrdBanner);
			}
		}
		...
		*/
		 ...
		 * @ngdoc-function
		 * @name goMkLink 
		 * 마이클코어스 메인 링크URL 처리 및 티클릭
		 * @param {object} banObj 배너 데이터
		 * @param {string} code 티클릭 코드 
		 * @param {string} idx 인덱스
		 * @param {string} subIdx 서브인덱스
		*/
		$scope.goMkLink = function(banObj,code,idx,subIdx){
			
			//티클릭 정의 
			var tclick =  $scope.tClickBase + "SpeMAll_MK_"; //base tclick;

			
			if(banObj.prdChk){ //상품배너 티클릭 재조합
				code = (code.indexOf('Ban') > -1) ? "Prod_Ban" : "Prod_Txt_";
			 }

			if(!idx){
				tclick += code; // 단일 배너
			}else{
				tclick += code + "0" + idx; // 리스트 배너
			}

			var path = banObj.linkUrl;
				path += ((banObj.linkUrl.indexOf("?") > -1) ? "&" : "?") +  $scope.baseParam + "&tclick=" + tclick;
	 ...
	 * @name specMkMallMain
	 * @description
	 * 마이클코어스 전문몰 분기 dirctive
	 * @example
	 * <spec-mk-mall-main></spec-mk-mall-main>
	 */
	app.directive('specMkMallMain', [ '$http','$window','LotteCommon','commInitData', 'LotteUtil', '$location','$timeout', function($http, $window, LotteCommon , commInitData, LotteUtil, $location, $timeout) {
        return {
            templateUrl : '/lotte/resources_dev/mall/michaelkors/spec_michaelkors_main.html',
            replace : true,
