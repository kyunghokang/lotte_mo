(function(window, angular, undefined) {
    ...
    app.controller('tvShoppingCtrl', ['$scope', '$http', '$filter', '$location', '$window', '$timeout', 'LotteCommon', 'LotteUtil', 'commInitData', 'LotteStorage', 'LotteUserService','$compile', function($scope, $http, $filter, $location, $window, $timeout, LotteCommon, LotteUtil, commInitData, LotteStorage, LotteUserService, $compile) { 
        $scope.showWrap = true;
        $scope.contVisible = true;
        $scope.ctgDispNo = "5537340"; // 카테고리 번호
        ...
        $scope.screenDataReset = function () { 
            ...
            $scope.screenData = {
                ...
                prod_list: [] // [deal] 딜 상품 리스트
            };
        };

        // 메인 탭 데이터 URL / Parameter 정의
        $scope.getThemaDataUrl = function (params) {
            var url = LotteCommon.tvShoppingData + "?";; // 데이터 호출 경로 설정
            ...
            if (LotteCommon.isTestFlag) { // 로컬 테스트 예외 처리
                ...
                url = LotteCommon.tvShoppingData; // 로컬 테스트 일 경우 추가 파라메타 제외
            }
            return url;
        };
        ...
        $scope.loadScreenData = function (params) {
            ...
            // Mixed Content (우선순위에 따라 배너/상품 조합 처리)
                ...
                if (!contents['prod_list'].items.length) {
                ...
                var sidx = ($scope.screenData.page - 1) * $scope.screenData.pageSize, // 배너 조합 시작 인덱스 지정 (page에 따라 시작 위치 조정)
                    productContent = $scope.screenData.prod_list, // 상품 데이터
                    ...
                    onair_prod = $scope.screenData.page == 1 ? true : false,
                    addItem = null,
                    i = 0,
        ...
        var tvShoppingLoc = LotteStorage.getSessionStorage('tvShoppingLoc');
        var tvShoppingDataStr = LotteStorage.getSessionStorage('tvShoppingData');
        var tvShoppingScrollY = LotteStorage.getSessionStorage('tvShoppingScrollY');

        if (tvShoppingLoc == window.location.href && tvShoppingDataStr != null && commInitData.query["localtest"] != "true") { // 현재 URL과 session storage에 저장된 경로가 동일 할 경우
            var tvShoppingData = JSON.parse(tvShoppingDataStr);

            $scope.screenData = tvShoppingData.screenData; // 화면 데이터 갱신
            $scope.pageLoading = false; // 페이지 로딩 중 Flag 감추기

            $timeout(function () {
                angular.element($window).scrollTop(tvShoppingScrollY);
            }, 800);
        ...
        angular.element($window).on("unload", function (e) {
            ...
            if (!commInitData.query.localtest && $scope.leavePageStroage) { // localtest url 파라메타가 없을때 저장
                LotteStorage.setSessionStorage('tvShoppingLoc', $location.absUrl());
                LotteStorage.setSessionStorage('tvShoppingData', sess, 'json');
                LotteStorage.setSessionStorage('tvShoppingScrollY', angular.element($window).scrollTop());
            }
        });
    }]);
    ...
    app.directive('lotteContainer', function() {
        return {
            templateUrl : '/lotte/resources_dev/mall/tvShopping_container.html',
            link: function ($scope, el, attrs) {
                $scope.getProductData = function () { // 상품 페이지 로드
                    ...
                    // [TCLICK] TV쇼핑추가로딩횟수
                    $scope.sendTclick('m_DC_TV_' + $scope.ctgDispNo + '_load' + ($scope.screenData.page - 1));
                };
