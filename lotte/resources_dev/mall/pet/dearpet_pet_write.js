(function( window, angular, undefined ) {
    ...
    app.controller('PetWriteCtrl', ['$scope', 'LotteCommon', 'commInitData', '$http', '$window', function($scope, LotteCommon, commInitData, $http, $window ) {
        ...
        $scope.test = commInitData.query['tt']!= null?true:false;;
        ...
        function myPetList( res ){
            ...
            for( var i=0; i<petLength; ++i ) {
                ...
                listData[formid] = {
                    ...
                    opt_brand : items[i].brand_cd,
                    opt:items[i].opt_val_cd,
                    cycle:items[i].cycle_cd                    
                };
                // 수정 대상 폼 인덱스
        ...
        function createForms() {            
            ...
            $scope.Request( {
                ...
                result : function(res){
                    ...
                    $scope.pageData = res.dearpet_baby_write;
            ...
            link : function(scope, el, attrs) {
                ...
                */
                ...
                scope.typeis = function( c, type ){
                    ...
                    scope.dataForm[c].opt_brand = "";
                    scope.dataForm[c].cycle = "";
                    scope.dataForm[c].opt = "";
                ...
                scope.optis = function( c, type ){
                    if( scope.dataForm[c].opt == type ){                        
                        scope.dataForm[c].opt = "";   //초기화                     
                    }else{
                        scope.dataForm[c].opt = type;                        
                    }
                }
                scope.cycleis = function( c, type ){
                    if( scope.dataForm[c].cycle == type ){
                        scope.dataForm[c].cycle = "";                            
                    }else{
                        scope.dataForm[c].cycle = type;    
                    }
                }
                ...
                */
                scope.photoPreview = function( target ) {
                    ...
                    loadImage( file, function (img) {
                            try{ 
                                scope.dataForm[i].photo = img.toDataURL();                                
                            }
                            catch(e) {                                 
                ...
                function defaultSetting(){
                    for( var i =0; i<scope.formList.length; ++i ) {
                        if(!scope.dataForm[ scope.formList[i] ]) {
                            ...
                            // 기본 선택 값
                            scope.dataForm[ scope.formList[i] ].opt_brand = "";
                ...
                function addFormSubmitEvent( type ) {
                    if( type == "bind" ) {
                        angular.element(".pet_write_forms form").bind('submit', function(e) {
                            ...
                            //신규 방식 20180906
                            var formid = angular.element(this).attr('dataID');
                            var fdata = scope.dataForm[formid];
                            //console.log("data ID", fdata.opt_brand, fdata.opt, fdata.cycle);
                            var formData = new FormData();                            
                                formData.append("mbr_no", getLoginInfo().mbrNo);
                                if(fdata.bbc_no){
                                    formData.append("bbc_no", fdata.bbc_no);
                                }                                    
                                if(fdata.type == '30'){
                                    formData.append("cate", fdata.type);
                                }
                                if(document.getElementById("file_" + formid).files[0] != undefined){
                                    formData.append("file", document.getElementById("file_" + formid).files[0]);
                                }
                                if(fdata.rotateCode != undefined){                                    
                                    formData.append("rotateCode", fdata.rotateCode);
                                }
                                if(fdata.opt_brand){
                                   formData.append("pur_brand", fdata.opt_brand);
                                }
                                if(fdata.opt){
                                   formData.append("pur_opt_val", fdata.opt);
                                }
                                if(fdata.cycle){
                                   formData.append("pur_cycle", fdata.cycle);
                                }
                                formData.append("name", fdata.name);
                                formData.append("type", fdata.type);
                                formData.append("size", fdata.size);
                                var vday = angular.element( "#date_" + formid).val();                                                            
                                vday = vday.replace(/[^(0-9)]/gi,"");
                                formData.append("birthday", vday);
                                formData.append("gender", fdata.gender);
                            
                                if(fdata.checklist){
                                    for(var i=0; i<fdata.checklist.length; i++){                                    
                                        if(fdata.checklist[i] != null){
                                            formData.append("worry[]", fdata.checklist[i].cd);           
                                        }                                   
                                    }                   
                                }
                                if(fdata.active == undefined){
                                    formData.append("active", false);                                
                                }else{
                                    formData.append("active", fdata.active);                                
                                }
                            
                            //var formData = new FormData( $(this)[0] );
                            var submitURL = ( fdata.bbc_no || fdata.bbs_no ) ? LotteCommon.petMallpetReWrite : LotteCommon.petMallpetWriteData;
                            ...
                            $.ajax({
                                ...
                                success: function( res ){
                                    //서버에서 결과 값이 문자열로 전달 됨.
                                    res = new String(res);
                                    if(res.indexOf('first') > -1){ //최초 등록 성공이면 
                                       scope.first_success = true;
                                    }
                                    resultStates.push( res.indexOf('reg_success') != -1 ? 'reg_success' : 'reg_error'  );
                                    //console.log( resultStates );
                                    if( resultStates.length == confirmCnt && scope.deleteResults.length == scope.deletePet_list.length ) {
                                        if(scope.first_success){
                                            alert("첫 번째 우리아이 등록 기념 쿠폰이\n발급되었습니다.\n마이롯데에서 확인하세요!");
                                            window.location.href = LotteCommon.petMallMainUrl+"?"+scope.baseParam; //메인으로
                                            return;
                                        }
                ...
                */
                function printState ( max, cusMsg ) {
                    ...
                    for( var i=0; i<resultStates.length; ++i ) {
                        ...
                        //20180830
                        /*
                        if(resultStates[i] != "first_reg_success"){
                            msg = "첫 번째 우리아이 등록 기념 쿠폰이 발급되었습니다.\n마이롯데에서 확인하세요!";
                        }
                        */
                    }
                ...
                */
                ...
                scope.submit = function(){
                    //이 아이로 활동하기 체크 : 체크된게 한개도 없으면 얼럿처리 
                    var active_ok = false;
                    for(var item in scope.dataForm){
                        var flag = scope.dataForm[item].active;
                        if(flag != undefined && flag){
                            active_ok = true;
                        }                        
                    }
                    if(!active_ok){
                        alert("대표로 활동할 아이를 선택해주세요.");
                        return;    
                    }                       
                    
                    scope.first_success = false;
                    // 전송중에 클릭 방지
	...
	app.filter('replacetag', [function() {
		return function(str) {
            str = str.replace("(", "<br>(");
            //str = str.replace("~", "<br>~");
			return str;
