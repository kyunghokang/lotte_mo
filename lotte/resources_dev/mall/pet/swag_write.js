(function(window, angular, undefined) {
    ...
    app.controller('SwagWriteCtrl', ['$http', '$scope', 'LotteCommon', 'LotteUtil', 'LotteForm', '$timeout', '$window', function($http, $scope, LotteCommon, LotteUtil, LotteForm, $timeout, $window) {
        $scope.showWrap = true;
        $scope.contVisible = true;
        $scope.subTitle = "뽐내기 작성"; // 서브헤더 타이틀
        $scope.screenID = "뽐내기 작성"; // 스크린 아이디 
    	...
    	angular.element(document).ready(function () {
    		$timeout(function(){
    			...
    			if($scope.appObj.isApp){
					...
					var offsetTop = parseInt(area.offset().top);
        			if($scope.appObj.isAndroid){
    					// android
    					try{
    						$window.lottepetswag.callAndroid('swag://position?top=' + offsetTop);
    					}catch(e){    						
    						//console.log('테스트', e);
    					}
    				}else{
    					//ios    					
    					location.href = 'swag://position?top=' + offsetTop;
        				// console.log("SEND APP TOP POSI", offsetTop);
    				}
        		}
        	}, 200);
    	});
    	...
    	$scope.cancel = function() {
    		if (confirm('뽐내기 작성을 취소하시겠습니까?')) {
				//alert("리스트 이동 할것")
    			location.replace(LotteCommon.petMallgalleryUrl+"?"+$scope.baseParam+"&curDispNo=5566438"+"&beforeNo=5564054"+"&cateDepth=2"+"&title=뽐내기");
    		}
    	};
    	...
    	$scope.save = function() {
    		...
    		LotteForm.FormSubmitForAjax(LotteCommon.swagWriteCheckData + "?swagTxt=" + reviewTxt)
    		.success(function(data) {
				if (data.chk_swag_cont.response_code != "0000") {
					...
					if ($scope.appObj.isApp) {
						...
						url += '&imageServerURL=' + "/mobile/swagPetImgParamReq.lotte";
						url += '&returnUrlData=' + "/mall/pet/dearpet_gallery.do"+ "?" + $scope.baseParam + "&curDispNo=5566438&beforeNo=5564054&cateDepth=2&title=뽐내기";
    ...
    app.directive('lotteContainer', ["$timeout", "$http", function($timeout, $http){
        return {
            templateUrl : '/lotte/resources_dev/mall/pet/swag_write_container.html',
            replace : true,
