(function(window, angular, undefined) {
    ...
    app.controller('DearpetEventCtrl', ['$http','$scope', '$window', '$location', 'LotteCommon', 'commInitData', 'LotteStorage', '$timeout', function($http, $scope, $window, $location, LotteCommon, commInitData, LotteStorage, $timeout) {
    	$scope.isMain = false;
    	$scope.showWrap = true;
    ...
    app.directive('lotteContainer',['$window','LotteCommon', '$timeout', function($window, LotteCommon, $timeout) {
        return {
            templateUrl : '/lotte/resources_dev/mall/pet/dearpet_event_container.html',
            replace : true,
		...
		return {
			...
			link: function ($scope, el, attrs) {
            	...
            	$scope.menuCategory1Click = function(item,index) {
            		var nEventExCode = "5566306"; // 신규 뉴스 추가 임시 코드
            		if( item.disp_no == nEventExCode ) return;
