(function(window, angular, undefined) {
	...
	app.controller('dotcomStaffBoardCtrl', ['$http', '$scope', 'LotteCommon', 'LotteForm', 'LotteUserService',
		...
		// console.log("STAFF", LotteUserService.getLoginInfo());
		// console.log("LotteUserService.getLoginInfo().isLogin", LotteUserService.getLoginInfo().isLogin);
		// console.log("LotteUserService.getLoginInfo().isDotStaff", LotteUserService.getLoginInfo().isDotStaff);
		// console.log(!LotteUserService.getLoginInfo().isLogin || !LotteUserService.getLoginInfo().isDotStaff);
		...
		// 닷컴 임직원 여부 확인
		// $timeout(function () {
		// 	if ($scope.loginInfo.isDotStaff != true) {
		// 		alert("임직원 전용 서비스 입니다.\r\n임직원 ID로 로그인 후 이용해 주세요.");
		// 		if($scope.appObj.isNativeHeader) {
		// 			appSendBack();
		// 		} else {
	...
	app.directive('lotteStaffContainer', ['LotteCommon', function(LotteCommon){
		return {
			templateUrl: '/lotte/resources_dev/custcenter/dotcomStaffBoard_container.html',
			replace: true,
