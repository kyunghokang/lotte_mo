(function(window, angular, undefined) {
    ...
    app.controller('OrderGiftCtrl', ['$scope','$filter','$sce','$http','$window','$log','LotteCommon', 'LotteUtil', function($scope , $filter , $sce , $http , $window , $log, LotteCommon, LotteUtil){
        $scope.showWrap = true;
...
//	var soilPointSearch = "N"; // soil포인트 조회 여부
	...
	function init() {
		...
		if($('#card_swipe').length){
			var crtIdx=0, $card=$('#card_swipe').parent(), ln=$('#card_swipe > li').length+2, dir=-1, $toLi=$('.msg_card .to_list');
			...
			$card.children('.btn_swipe').click(function(){
				...
				var wd=$card.width(), sx=cardSwipe._x()+wd*dir;
			...
			function msgSwipeEnd(){
				var wd=$card.width(), idx=cardSwipe._x()/-wd, cardMsg,
		...
		if($('#premium_detail_list').length){
			var $pBtn=$('.layer_gift_pop.view_premium .swipe_wrap'), $pWrap=$('#premium_detail_list').parent(), prmLn=$('#premium_detail_list > li').length+2, pDir=-1;
			...
			$pBtn.children('.btn_swipe').click(function(){
				...
				var prmWd=$pWrap.width(), prmSx=premiumSwipe._x()+prmWd*pDir;
		...
		$('.pop_layer_pop').on('click',function(){
			var clsNm=$(this).attr('class').match(/\b(view_).*?\b/),
				...
				$('#pageCover').show();
				$(popNm).css({'top':0,'bottom':'auto'}).addClass('on');
		...
		if($('#premium_list').length){
			var $pList=$('#premium_list'), pLn=$pList.children('label').length, listW=$pList.children('label').eq(0).outerWidth(true)*pLn;
			$pList.width(listW);
			var $pdrag = $pList.simpleDrag(function(){
					var wd = $pList.parent().width(), curwd = $pdrag._width()-wd, curpos = $pdrag._x();
...
*/		
	...
	function setMaxDc() {
		...
		$("#frm_inp select[name=coupon]").find("option").each(function(){
			...
			if ( coupon_idx != "N" ) { // 1차 선택
				...
				if ( $.type($("#frm_inp select[name=dup_coupon]").val()) == "undefined" || dlv_cnt > 1) { // 2차가 없다. && 배송지 2군데 이상
				...
				if ( $.type($("#frm_inp select[name=dup_coupon]").val()) != "undefined" && dlv_cnt == 1) { // 2차가 있다. && 배송지 1군데
					$("#frm_inp select[name=dup_coupon]").find("option").each(function(){
						var dupcpn_idx = $(this).val();
	...
	function fn_cardsavegoods(){
		...
		$.ajax({
			...
			data: {goods_no_list:v_goods_no_list, card_knd_cd:card_cd},
