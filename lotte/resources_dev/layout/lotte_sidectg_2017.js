(function (window, angular, undefined) {
	...
	sideCtgModule.directive('lotteSideCategory', ['$window', 'LotteCommon', function ($window, LotteCommon) {
		return {
			templateUrl: '/lotte/resources_dev/layout/lotte_sidectg_2017.html',
			replace: true,
			...
			link: function ($scope, el, attrs, ctrl) {
				...
				$scope.tabChange = function (type) {
					...
					if(type == 'brd') {
						$window.location.href = LotteCommon.brandSearchUrl+"?"+$scope.baseParam+"&tclick="+tclick;
					} else {
						$scope.sendTclick(tclick);
