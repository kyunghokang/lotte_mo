(function(window, angular, undefined) {
	...
	var customSearchModule = angular.module('customSearch', [
		'lotteComm'        
	]);

	customSearchModule.controller('CustomSearchCtrl', ["$scope", "LotteCommon", function ($scope, LotteCommon){}]);

	customSearchModule.directive('customSearch', ['LotteCommon', 'LotteStorage','$window', '$timeout', '$parse', '$http', function (LotteCommon, LotteStorage, $window, $timeout, $parse, $http) {
		return {
			templateUrl : '/lotte/resources_dev/layer/custom_search.html',
			replace : true,
			link : function($scope, el, attrs) {
				...
				$scope.csCloseCustomSearch = function(){
					...
					if($scope.LotteDimm.target == "customSearch") {
						$scope.dimmedClose();
					}else{
						$scope.csUIStatus.visible = false;
					}

					/*if($scope.appObj.isApp && $scope.appObj.isIOS){
						//$timeout(function(){
							//액션바 보이기
							$window.location.href = "lottebridge://lotteapps/show";
						//}, 100);
					}*/
				}
				$scope.csShowHideCustomSearch = function(flag){
					if($scope.csUIStatus.visible) {
						$scope.csCloseCustomSearch();
					}else{
						$scope.csUIStatus.visible = true;
						...
						//$scope.initCustomSetting();
						$scope.csInitTempSettings();
						$scope.dimmedOpen({
							target: "customSearch",
							callback: this.csCloseCustomSearch,
						...
						if(flag == "list"){
							$scope.sendTclick($scope.tClickBase + "SrhResult_Clk_customizing_search_Btn");
						}else if(flag == "layer"){
							$scope.sendTclick($scope.tClickBase + "SrhLayer_Clk_customizing_search_Btn");
						}
						...
						if($scope.appObj.isApp){
							if($scope.appObj.isAndroid){
								//액션바 감추기
								/*$timeout(function(){
									$window.location.href = "lottebridge://lotteapps/hide";
								}, 1000);*/
