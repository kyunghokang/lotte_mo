		...
		return function (scope, el, attrs) {
			...
			function dragEvent(e) {
				...
				switch(e.type){
					case "touchstart":
						...
						config.x = touchX;
						config.y = dy;
						config.touchtime = new Date().getTime();
						break;
					case "touchmove":
					...
					case "touchend":
						var maxtime = 500,
							endtime = Math.abs(new Date().getTime()-config.touchtime),
			...
			function draging( x, e ) {
				...
				if(!e) {
					...
					if((config.hold>=0&&cposx>=0)||(config.hold<=-(itemsWidth-limit)&&cposx<=-(itemsWidth-limit))) config.speed = 300;
