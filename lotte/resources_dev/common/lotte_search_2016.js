(function(window, angular, undefined) {
    ...
    srhModule.directive('lotteSearch', ['LotteUtil', 'LotteCommon', 'LotteStorage', '$sce', '$window', '$timeout', '$http', function(LotteUtil, LotteCommon, LotteStorage, $sce, $window, $timeout, $http) {
        return {
            templateUrl: '/lotte/resources_dev/layer/layer_search_2016.html',
            ...
            link:function($scope, $el, attrs) {
                ...
                $scope.closeSrhLayor = function(tclick) {
                    ...
                    if(tclick == "tclick"){
                    	$scope.sendTclick( $scope.tClickRenewalBase + "SrhLayer_Clk_Close" );
                    }
                };
                ...
                $scope.showRecentKeyword = function(){
                    ...
                    ////////////////////////////////// modify recent search 2016.10 /////////////////////////////////////////
					$scope.recentKwData = [];
					
                    var myWord = localStorage.getItem("myWord2016");
                    if(myWord == null || myWord == ""){
                    	...
                    	if(myWord == null || myWord == ""){
                    		...
                    		// OLD
                    		var arr = myWord.split("|");
                    		angular.forEach(arr, function(word){
                    			$scope.recentKwData.push({keytxt:word, date:""});
                    		});
                    		saveRecentKeywordLS();
                    	...
                    	for(var i=0; i<len; i++){
                    		if(obj.list[i].$$hashKey != undefined){
                    			delete obj.list[i].$$hashKey;
                    		}
                    	}
                ...
                function saveRecentKeywordLS(){
                    if($scope.recentKwData.length > 20){
                    	$scope.recentKwData.length = 20;
                    }
					...
					localStorage.setItem("myWord2016", json);
                }
                
                /**
                 * 최근 검색어 다시 검색
                 */
                $scope.researchRecentKeyword = function(e, tclick){
                	var keyword = angular.element(e.currentTarget).find("em").text();
                 ...
                 * 인기/급상승 검색어 검색
                 */
                $scope.searchBestKeyword = function(keyword, tclick){
                	$scope.keyword = keyword;
                	$scope.saveRecentKeyword();
                	$scope.fn_goSearch(keyword, tclick);
                };
                ...
                $scope.delRecentAllKeyword = function(){
                    if (confirm('최근 검색어를 모두 삭제 하시겠습니까?') == true){
                        //localStorage.removeItem('myWord');
                        localStorage.removeItem("myWord2016");
                        $scope.recentKwData = undefined;
                ...
                */
                ...
                $scope.customSearchPage = function(type) {
                    ...
                    if(type == "srhResult"){
                        tclick = "SrhResult_CLK_customizing_search_Btn";
                    }else{
                        tclick = "SrhLayer_Clk_customizing_search_Btn";
                    }
