(function(window, angular, undefined) {
        ...
        return {
            ...
            link:function($scope, el, attrs) {
                ...
                $scope.clickUnit = function() {
                    // 20150824 박형윤 - 리뉴얼쪽에는 인입전시코드가 상품데이터에 있어 같이 맞추기 위하여 수정
                    $scope.item.curDispNo = curdispno;
                    $scope.item.curDispNoSctCd = curdispnosctcd;
    ...
    unitModule.directive('lotteUnit01ImgSrc', ['$http', 'LotteCommon', function ($http, LotteCommon) {
        return {
            link : function ($scope, el, attrs) {
                     if (($scope.item.byrAgelmt == "19" && !$scope.loginInfo) || (!$scope.loginInfo.isAdult && $scope.item.byrAgelmt == "19")) { /*19세 상품 확인*/
    ...
    unitModule.directive('commUnitType01', ['$http', 'LotteUtil', 'LotteCommon', 'LotteLink', 'LotteStorage', function ($http, LotteUtil, LotteCommon, LotteLink, LotteStorage) {
        return {
            ...
            link: function ($scope, el, attrs) {
                ...
                $scope.addWish = function () { /*위시리스트 상품 담기*/
                    if ($scope.item.outlnkMall == "LECS") { /*LECS 상품*/
                    ...
                    $scope.sendProductWish($scope.item.goodsNo, function (res) {
                        /**
                        1. 이미 등록된 상품입니다.
                        2. 선택하신 상품은 \n상품정보화면에서 담아주세요.
                        3. 죄송합니다. 품절된 상품입니다.
