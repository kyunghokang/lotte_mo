...
(function (window, angular, undefined) {
	 ...
	 * 공통 모듈
	 */
	commModule = angular.module('lotteComm', ['ngSanitize', 'lotteUtil', 'lotteFilter', 'lotteUrl']);
	...
	commModule.controller('LotteCtrl', ['$scope', '$parse', '$location', '$window', '$document', '$http', '$compile', '$timeout', 'LotteUtil', 'LotteLink', 'commInitData', 'LotteCommon', 'LotteUserService', 'LotteForm', 'LotteStorage', 'LotteCookie',
        ...
        */
        ...
        $scope.trace = function(){
        	...
        	var str = JSON.stringify(args, null, " ");
        	...
        	if(wrap.length == 0){
        		...
        		query += '<a onclick="angular.element(\'#traceWarp\').hide();" style="display:block;width:40px;height:24px;position:absolute;top:-24px;right:0;background-color:#fff;box-shadow:inset 0 0 0 1px #333;letter-spacing:-1px;text-align:center;line-height:24px;">Close</a>';
        		query += '<a onclick="angular.element(\'#traceWarp > div\').empty();" style="display:block;width:40px;height:24px;position:absolute;top:-24px;right:39px;background-color:#fff;box-shadow:inset 0 0 0 1px #333;letter-spacing:-1px;text-align:center;line-height:24px;">Clear</a>';
        		query += '<div style="width:100%;height:100%;overflow:auto;"></div></div>';
        		wrap = angular.element(query);
        		angular.element("body").append(wrap);
        	}
        	wrap.show();
        	wrap.find("> div").prepend('<div>' + str + '</div>');
        };
	 ...
	 */
	commModule.directive('lotteBtntop', ['$window', 'LotteCommon', function ($window, LotteCommon) {
		return {
			template: '<div class="quick_btn"><div ng-show="visible"><a class="btn_back" ng-click="gotoPrepageSide()" ng-show="quickBackBtn">뒤로</a><a class="btn_top" ng-click="gotoTop()">위로</a></div><a ng-show="isShowBtnTalk" class="btn_talk" ng-click="gotoTalk()">톡상담</a></div>',
			replace: true,
			link: function ($scope, el, attrs) {
...
*/
	...
	commModule.directive('subHeaderSearch', ['$window', '$timeout', function ($window, $timeout) {
		return {
			templateUrl: '/lotte/resources_dev/layout/sub_header_search_2016.html',
			replace: true,
			link: function ($scope, el, attrs) {
				// 이전 페이지 링크
				$scope.gotoPrepage = function () {
            ...
            link : function($scope, el, attr){
                ...
                function setSize(time){ //초기화              
                    ...
                    $timeout(function(){
                        ...
                        //console.log("len", len, rolling, childTotal, others);
	 ...
	 */
	commModule.directive('keepPopup', ['$compile','$window','LotteStorage','$http','LotteCommon','LotteCookie', function ($compile, $window,LotteStorage,$http,LotteCommon,LotteCookie) {
		return {
