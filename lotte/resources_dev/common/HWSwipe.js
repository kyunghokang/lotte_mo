...
(function(){
     ...
     * - 전용 style추가
     */
    .run(['addRule',function(addRule){
        addRule('.hwswipe_clear:after','content:"";padding:0; margin:0; display:block;clear:both;');
        //console.log("%cHWSWIPE", "background: white; color: #e68a97; font-size: 11px; font-weight: 400");
    }])
    
    .factory('addRule',[function(){
        return function(id, newStyle){
            try{
                document.styleSheets[0].addRule(id,newStyle);
            } catch(e){
                document.styleSheets[0].insertRule(id +"{"+ newStyle +"}");
            }
        }
    }])
     ...
     * @name hwSwipeCtrl
     */
    .controller( 'hwSwipeCtrl', ['$scope','addRule',function($scope,addRule){
        this.addStyle =  addRule;
        ...
        return {
            ...
            link : function(scope,el,attrs,ctrl){
                ...
                function copyItems( index, str ){
                    ...
                    if(!option.infinity) return;//item.css({opacity:0});
                    if(str=="next") {
                        el.append( item );
                    } else {
                        el.prepend( item );
                ...
                function moveX(index,easing,apply) {
                    ...
                    if( index>-1&&index<option.total) {
                        posx = -((option.infinity?option.parentWidth:0)+option.posx[index][0].left);
                        if(index==option.total-1){
                            var basew = (option.infinity?el.children()[0].offsetWidth:0)-option.marginRight;
