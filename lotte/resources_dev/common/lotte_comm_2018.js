...
(function (window, angular, undefined) {
	...
	commModule.service('LotteUserService', ['$http', '$timeout', '$q', 'LotteCommon', '$sessionStorage', 'commInitData', 'LotteStorage', 'LotteCookie', '$interval', 
		...
		*/
		this.callSsoLogin = function(Loginchk, LoginId, initAcesTkn){
			...
			var simpleChk = (LotteCookie.getCookie("SIMPLESIGNYN") == 'Y') ? true : false; // 간편로그인 체크값
			// sso 초기화 세팅 후 호출
			self.getSsoInitData(initAcesTkn).then(function(){
				
				//URL 파라미터에 SSO토큰이 있거나 로그인이 아니거나 로그인인데 접근토큰이 없고 간편로그인이 아닐때  로그인 페이지가 아니면   

				if((!LoginId || ssoTkn || (LoginId && !ssoAcesTkn)) && location.pathname != "/login/m/loginForm.do" && !simpleChk ){ // 20181230_1
		...
		*/
		this.ssoLoginResult = function(loginRes,type,resDta){
			...
			if(loginRes.rspC == "00"){ //로그인 정상 처리
				sso = new SsoClientLibrary({ // 전달받은 AccessToken으로 라이브러리 재 초기화 20181230_1
					ccoSiteNo : '1105',
				...
				switch(loginRes.rspDtc){
					...
					case "404": //부정로그인 대상
						...
						ssoData= {
									...
									onlCstTpc : loginRes.onlCstTpC //온라인고객유형코드
						}
					break;
		...
		/**
		 * @ngdoc function
		 * @name LotteUserService.callEnableTkn
		 * @description
		 * 로그인 중 SSO 접근토큰 만료 체크 - 만료시 새로 발급된 접근토큰으로 재 초기화 
		*/
		this.callEnableTkn = function(){
			var deferred = $q.defer();
			var logined = LotteCookie.getCookie("LOGINID"); // 로그인상태 체크 쿠키값
			var acess = LotteCookie.getCookie("SSO_ACES_TKN"); // 갱신토큰 쿠키값
			
			// 로그인 중이고 갱신토큰 주기가 세팅되어 있으면 
			if(logined && acess){ // L.POINT 로그인중일때

				// 갱신토큰 유효성 체크를위해 사용자 정보 호출시도
				sso.callApi({
					akUrl: '/biz/login/urInfInq_01_001',    // 요청URL
					callback: function(rspDta) {						
						if (rspDta.rspC != "00") {
							//갱신토큰 만료(1주일)시 토큰 발급 후 세팅
							self.callAutoSsoLogin().then(function(autoRes){
								// 갱신,접근토큰 재발급 완료 후
								deferred.resolve();
							}).catch(function(autoEr){
						...
						}else{// 현재 갱신토큰이 유효하다면 pass
							deferred.resolve();
						}
					}
				});

			}else{ // 로그인 아닐때나 간편회원일때
				deferred.resolve();
			}
		return deferred.promise;
		}
		...
		*/
		this.callSsoScreen = function(akUrl,rturUrl,data){
			...
			if(ssoAlive){ // 멤버스 서버 상태 체크값
				...
				try{
					self.callEnableTkn().then(function(){
						// sso 화면호출 시도
						sso.callScreen(opt);
					}).catch(function(){
						console.log("[SSO 통합회원제 접근토큰 만료 화면 호출 불가]");
					});
				}catch(er){
	...
	commModule.controller('LotteCtrl', ['$scope', '$parse', '$q', '$location', '$window', '$document', '$http', '$compile', '$timeout', 'LotteUtil', 'LotteLink', 'commInitData', 'LotteCommon', 'LotteUserService', 'LotteForm', 'LotteStorage', 'LotteCookie', 'LotteGA', 
		...
		// 20190110 로그인후 뒤로가기 정책 수정건 afterjob
		function checkDevice(name) {
			var result = '';
			if (name !== '') {
				var matchKw = navigator.userAgent.match(name);
				if (matchKw !== null) {
					result = matchKw[0];
				}
			}
		...
		if (checkDevice('iPhone')) {
			...
			$scope.appObj.iOsType = deviceName;
		...
		*/
		 ...
		 * 2018.12.27 최재훈 - 어도비 라이브러리 로드시 깜밖임 이슈로 meta 파일에서 라이브러리 로드
		 */
		...
		function setGroobee(){
			...
			// 20181228
			var groobeeMeta = angular.element("meta[property*='groobee:']");
			if(groobeeMeta.length > 0) { groobeeMeta.remove(); } 
		...
		$scope.$watch('loginInfo',setGroobee);  // 20181228
			...
			link : function($scope, el, attrs) {
				...
				$scope.gotoPrepage = function () {
					...
					if($scope.appObj.isNativeHeader) {
						...
						history.back(); // 20190103 로그인 뒤로가기 개선
					}
				};
