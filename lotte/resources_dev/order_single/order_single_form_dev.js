...
function fn_setDisplayAssentArea(pg_display){
	if ($(".box_agree_check.assent_all_wrap").length > 0){
		if (pg_display=='Y'){
			$(".box_agree_check").hide();
			$(".box_agree_check.assent_all_wrap").show();
		}else{
			$(".box_agree_check").show();
		...
		if('Y' == useDlvHopeDdArr[i]){	// 희망 배송일 사용 일때			
			if('' == $("#select_dd").val() || '날짜를 선택해 주세요.' == $("#select_dd").val()){
				...
				$.ajax({
					...
					dataType:"json",
					cache:false,
					data:{mdlNo : mdlnoArr[i], qty : ordqtyArr[i], dlvHopeDd : $("#select_dd").val()},
...
function lpay_card_list(){	
	...
	$("#frm_inp select[name=iscmcd] option").each(function(){
		...
		if(card_cd != ""){
			...
			lpay_html += "		<input type='radio' id='rdo_lpay_idx_"+lpay_idx+"' name='lpay_type' value='"+$(this).val()+"' onClick='lpay_card_click(\""+$(this).attr("data-card-id")+"\")' data-card-nm='"+$(this).attr("data-card-nm")+"'  class='radio01' data-card-id='"+$(this).attr("data-card-id")+"'>";
			lpay_html += "		<label for='rdo_lpay_idx_"+lpay_idx+"' class='ng-binding'>"+$(this).text()+" </label>"
						 + "	</radiobox>			"
						 + "</radiobox-list>";
...
function lpay_card_click(id){
	...
	if($("#frm_inp select[name=iscmcd] option[data-card-id="+id+"]").data("card-div") != "CC"){
		$("#frm_inp select[name=cardinstmon] option:eq(0)").prop('selected' , true);	// 일시불선택
		$("#frm_inp select[name=cardinstmon]").prop('disabled' , true);	// 비활성 처리
	}
}
...
//							|| !chkEmpty($("#frm_inp select[name=bankno]"))  || $("#frm_send input[name=cardkndcd]").val() != "" || $("#frm_send input[name=cardkndcd_dup]").val() != "" )){
	...
	if($("#frm_send input[name=cardkndcd]").val() == "" && $("#frm_send input[name=cardkndcd_dup]").val() == ""){
		// 보관금 / 엘머니 체크
	...
	if ( histPayMeanCd == PAYTYPE_CODE_CARD ) {
		...
		$("#hist_paymean").addClass('check');
		$("#hist_paymean").addClass('on');
	} else if ( histPayMeanCd == PAYTYPE_CODE_LPAY ) {
		$("#frm_inp input[name=rdo_paytype]:radio[value='"+PAYTYPE_CODE_LPAY+"']").click();
...
function fn_all_dc(){
	...
	//console.log("1차 : "+$("#frm_send input[name=prommdclcd]").val());
	//console.log("2차 : "+$("#frm_send input[name=prommdclcd_dup]").val());
...
function lpayView(obj){
	...
	lpay_html	+="<div class='l_pay_logo'>"
					...
					+ "</div>"
					+ "<div class='check_img'>"
					+ "    <img src='http://image.lotte.com/lotte/mo2015/angular/order/pay_check.png' alt=''>"
					+ "</div>"
					+ "<div class='s_text'>"
					+ "	   <span class='s_box02'>"
...
function init_payMean(){
	...
	if(histPayMeanCd == PAYTYPE_CODE_CARD){	//카드
		//카드목록에 있는경우 처리
		if(hist_cardNm != "" && creditPsbYn == "Y"){
			...
			paymean_html +="	<div class='card_name'>"
							+"		<p class='title1'>[최근]"+hist_cardNm+"</p>"
	...
	}else if(histPayMeanCd == PAYTYPE_CODE_BANK){	//무통장
		if(bankPsbYn == "Y"){
	...
	}else if(histPayMeanCd == PAYTYPE_CODE_PHONE){	//휴대폰
		if(sttlPsblPhone == "Y"){
			paymean_html ="<div class='card_name'>"
						 +"    <p class='title1'>[최근]휴대폰결제</p>"
						 +"</div>"
						 +"<div class='check_img'>"
						 +"    <img src='http://image.lotte.com/lotte/mo2015/angular/order/pay_check.png' alt=''>"
						 +"</div>"
		...
		if($(this).val() != "" && lpay_id == ""){
			if($("#frm_inp input[name=op_pay_mean_hist_acqr_cd]").val() == $(this).val() && $("#frm_inp input[name=op_pay_mean_hist_pay_mean_cd]").val() == PAYTYPE_CODE_LPAY){
				lpay_html    += "<li id='lpay_paymean_"+$(this).attr("data-card-id")+"' onClick='paySelect(\"lpay_paymean_"+$(this).attr("data-card-id")+"\",1);' class='lpay'>"
							  ...
							  + "</div>"
							  + "<div class='card_name'>"
							  + "	<p class='title1'>[최근]"+$(this).attr("data-card-nm")+"</p>"
