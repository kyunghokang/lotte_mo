(function (window, angular, undefined) {
	...
	app.controller('MainCtrl', ['$rootScope', '$scope', '$http', '$timeout', '$window', '$location', 'LotteCommon', 'commInitData', 'LotteStorage', 'LotteCookie', 
		function ($rootScope, $scope, $http, $timeout, $window, $location, LotteCommon, commInitData, LotteStorage, LotteCookie) {
		...
		$scope.screenDataReset = function () { // 탭 콘텐츠 데이터의 데이터 정의 초기화
			...
			$scope.screenData = {				
				...
				platinum_banner : {} //플래티넘 대상 코너화 배너 20160204	 eung
			};
		};
		...
		return {
			link: function ($scope, el, attrs) {
				...
				$scope.goStyleBrand = function (brandNo, tclick) { // 브랜드 매장 연결
					if (brandNo) {
						var url = LotteCommon.brandShopUrl + "?" + $scope.baseParam + "&upBrdNo=" + brandNo;
