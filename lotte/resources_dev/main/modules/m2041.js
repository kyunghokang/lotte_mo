...
(function(window, angular, undefined) {
	...
	app.directive('m2041Container', ['$timeout', '$window', '$location', 'LotteCommon', 'LotteCookie', 'LotteStorage', 'LotteUtil',
	function ($timeout, $window, $location, LotteCommon, LotteCookie, LotteStorage, LotteUtil) {
		return {
			restrict: 'AEC',
			scope: { moduleData : "=" },
			templateUrl : "/lotte/resources_dev/main/modules/m2041.html",
			replace : true,
			...
			link : function ($scope, el, attrs) {
				...
				$scope.logGAEvtModuleEach = function (idx, labelTxt, tabNm, subIdx) {
					...
					if (idx != "-") {
						index = (subIdx) ? "탭_" + tabNm + "_배너_" + LotteUtil.setDigit(subIdx): "탭_" + labelTxt +"_"+ LotteUtil.setDigit(idx);
					}
					label = (label + "").replace(/\n/gi, " ");
