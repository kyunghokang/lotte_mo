...
(function(window, angular, undefined) {
	...
	app.directive('m2057Container', ['$timeout', '$window', '$location', 'LotteCommon', 'LotteCookie', 'LotteStorage', 'LotteUtil',
	function ($timeout, $window, $location, LotteCommon, LotteCookie, LotteStorage, LotteUtil) {
		return {
			restrict: 'AEC',
			scope: { moduleData : "=" },
			templateUrl : "/lotte/resources_dev/main/modules/m2057.html",
			controller: 'lotteModulesCtrl',
			replace : true,
			link : function ($scope, el, attrs) {
				$scope.rScope = LotteUtil.getAbsScope($scope);
				$scope.fontColor = $scope.rScope.mainData.colorInfo.fontColor;
				$scope.lineColor = $scope.rScope.mainData.colorInfo.lineColor;
				console.log("$scope.lineColor", $scope.lineColor);
				...
				$scope.tabClick = function (e, index) {// 탭 클릭
					$scope.moduleData.uiData.tabIdx = index;
					$scope.rScope.sendTclick($scope.moduleData.tclick + '_' + this.item.dispNo + '_Swp_Bigcate');
				...
				$scope.logGAEvtModuleEach = function (idx, labelTxt, tabNm, subIdx) {
					...
					if (idx != "-") {
						index = (!subIdx) ? "탭_"+ label + "_" +  LotteUtil.setDigit(idx) : "탭_" + tabNm + "_상품_" + LotteUtil.setDigit(subIdx) ;
					}
