...
(function(window, angular, undefined) {
	...
	app.directive('m2058Container', ['$rootScope', '$http', '$timeout', '$window', '$location', 'LotteCommon', 'LotteCookie', 'LotteStorage', 'LotteUtil','commInitData',
	function ($rootScope, $http, $timeout, $window, $location, LotteCommon, LotteCookie, LotteStorage, LotteUtil,commInitData) {
		return {
			restrict: 'AEC',
			scope: { moduleData : "=" },
			templateUrl : "/lotte/resources_dev/main/modules/m2058.html",
			replace : true,
			...
			link : function ($scope, el, attrs) {
				...
				var jsonCallUrl = $scope.moduleData.linkInfo.couponUrl;

				$scope.loadData = function(params) {
					var jsonUrl = LotteCommon.isTestFlag ?  "/json/main/main_m2058.json" : jsonCallUrl;
					var httpConfig = {
						method: "get",
						url: jsonUrl,
						params: params
				...
				$scope.couponDown = function(tclick) {
					...
					var jsonUrl = LotteCommon.isTestFlag ?  "/json/main/main_m2058_cpnDown.json" : $scope.callBackData.cpnDownUrl;
					var httpConfig = {
						method: "get",
						url: jsonUrl
						// params: params
