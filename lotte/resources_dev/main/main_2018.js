(function (window, angular, undefined) {
	...
	app.controller('MainCtrl', ['$rootScope', '$http', '$compile', '$scope', '$timeout', '$window', '$location', '$q', 'LotteCommon', 'commInitData', 'LotteUserService', 'LotteStorage', 'LotteCookie', 'AppDownBnrService','LotteUtil', 'LotteGA', 
		function ($rootScope, $http, $compile, $scope, $timeout, $window, $location, $q, LotteCommon, commInitData, LotteUserService, LotteStorage, LotteCookie, AppDownBnrService, LotteUtil, LotteGA) {
        ...
        //문서 로드 후 세팅    
        $scope.gnbMenuOpen = function(flag){              
		...
		$scope.moveMenu = function (dispNo, disableAnimation) {
			...
			if($scope.pageUI.rootMenu[$scope.pageUI.curMenuIdx].subTab) {
				...
				//20180813 직접랜딩시 서브탭 태깅
				if($scope.pageUI.cateDirectYn){	

					// eventCategory, eventAction, eventLabel, dimension53, dimension54
					LotteGA.evtTag("MO_공통_헤더", "메인탭", subSelectedItem.dispNm, "", "");
					$scope.pageUI.cateDirectYn = false; // 직접랜딩 false
				}
        ...
        //오픈 메뉴에서 GA 태깅 추가     
		$scope.logGAEvtOpenMenu = function (evtLabel) {	
			LotteGA.evtTag("MO_공통_헤더", "메인탭_펼쳐보기", evtLabel, "", "");
		};            
		...
		$rootScope.mainSwipeReset = function ($event) {
			...
			$scope.logGAEvtComm('cateSwipe',$scope.pageUI.rootMenu[targetIdx].dispNm); //20180419 GA 이벤트 태깅
		...
		$rootScope.loadData = function (page, dispNo, target, tabIndex) {
            ...
            //201811 빅딜에서 서브카테고리용으로 추가 
            if(commInitData.query["ctgDispNo"]){
                httpConfig.params.ctgDispNo = commInitData.query["ctgDispNo"];    
                httpConfig.params.rowsPerPage = 30;                
            }
			...
			.finally(function () {
				$scope.ajaxLoadFlag = false;                
                if(httpConfig.params.ctgDispNo){ //201811 빅딜에서 서브카테고리용으로 추가 
                    $scope.mainData.moduleData[0].ctgDispNo = httpConfig.params.ctgDispNo;
                    if($scope.mainData.moduleData.length == 2){ //오늘의 L 때문에 추가함
                        $scope.mainData.moduleData[1].ctgDispNo = httpConfig.params.ctgDispNo;   
                    }
                }                
	 ...
	 * 모듈내 버튼 클릭 등 처리
	 */
	app.controller('lotteModulesCtrl', ['$rootScope', '$scope' , '$window', 'LotteCommon', 'LotteUtil', 'LotteGA', 
	function($rootScope, $scope, $window, LotteCommon, LotteUtil, LotteGA) {
		...
		$scope.mainProductClick = function(goodsNo, tclick) {
			if(goodsNo) {
				var url = LotteUtil.setUrlAddBaseParam(LotteCommon.productviewUrl , $rootScope.pageUI.baseParam + "&goods_no=" + goodsNo + "&curDispNo=" + $rootScope.pageUI.curDispNo + "&curDispNoSctCd=" + $rootScope.pageUI.curDispNoSctCd);
        ...
        //201804 광고솔루션 추가 
        $scope.calllmp = function(code){
            //console.log("calllmp", code);
            new Image().src = LotteCommon.adReqMain + "imp?code=" + code;
        }        
		$scope.mainBannerClick = function(linkUrl, isOutLink, tclick, ad_code) {            
			if(linkUrl) {
                if(ad_code){//광고솔루션 여부 
                    //alert(ad_flag);
                    new Image().src = LotteCommon.adReqMain + "click?code=" + tclick;
                    console.log("광고솔루션 code :", tclick);
                }
                setTimeout(function(){
                    if(isOutLink) {
                        window.open(linkUrl);
				...
				//console.log("Main Banner Click : ", ad_code, "\n tclick : " ,tclick);
			}
		}
		...
		$scope.logGAEvtModule = function (groupModuleNm, idx, label, tabNm) {            
			var gaCtg = "MO_메인",
				subTabNm = "",
				gaGroupNm = groupModuleNm ? groupModuleNm : "",
				gaIdx = idx ? idx : "-",
				gaLabel = label ? label : "-",
				gaModuleNm = "-";

			gaCtg = "MO_메인_" + $rootScope.pageUI.rootMenu[$rootScope.pageUI.curMenuIdx].dispNm;
			
			// 선택된 서브 탭이 있는지 확인하고 이름 붙이기
			if ($rootScope.pageUI.rootMenu[$rootScope.pageUI.curMenuIdx].subTab &&  $rootScope.pageUI.rootMenu[$rootScope.pageUI.curMenuIdx].subTab.items) {
				angular.forEach($rootScope.pageUI.rootMenu[$rootScope.pageUI.curMenuIdx].subTab.items, function (value, key) {
					if (value.dispNo == $rootScope.pageUI.curTabMenuDispNo) {
						subTabNm = value.dispNm;
					}
				});
			...
			if (gaGroupNm.indexOf("|") > -1) {
				tmpGAGroupData = (gaGroupNm + "").split("|");

				if (tmpGAGroupData && tmpGAGroupData.length == 2) {
					gaGroupNm = tmpGAGroupData[0];
					gaModuleNm = tmpGAGroupData[1];
				} else {
					gaGroupNm = tmpGAGroupData[0];
				}
			}
		...
		$scope.logGAEvtTab = function (idx, label) {
			var gaCtg = "MO_메인",
				subTabNm = "",
				gaGroupNm = label,
				gaIdx = idx ? idx : "-",
	...
	app.controller('lotteHeaderMainCtrl', ['$scope', '$window', '$http', '$timeout', 'LotteCommon', 'LotteCookie', 'commInitData', 'LotteStorage',
		...
		//펼쳐진 상단메뉴용 데이타 생성하기 20181126
        function makeRootMenu(){
            /*
            var tmp = [];
            for(var i=0;i<$scope.pageUI.rootMenu.length; i++){
                if($scope.pageUI.rootMenu[i].subTab){
                    for (var j = 0; j < $scope.pageUI.rootMenu[i].subTab.items.length; j++) { 
                        $scope.pageUI.rootMenu[i].subTab.items[j].mtype = "sub";
                        $scope.pageUI.rootMenu[i].subTab.items[j].tabidx = j;
                        $scope.pageUI.rootMenu[i].subTab.items[j].parDispNo = $scope.pageUI.rootMenu[i].dispNo;
                        tmp.push($scope.pageUI.rootMenu[i].subTab.items[j]);
                    }                        
                }else{
                    $scope.pageUI.rootMenu[i].mtype = "main";
                    tmp.push($scope.pageUI.rootMenu[i]);
                }
            }
            $scope.pageUI.rootOpenMenu = tmp;            
            */
            var menu_slider = angular.element('nav[lotte-slider]');
            setTimeout(function () {
                menu_slider.scope().reset();
                console.log("menu_reset");
            },300);                        
            
            //서브탭 비노출로 변경
            $scope.pageUI.rootOpenMenu = $scope.pageUI.rootMenu; 
            $timeout(function(){
                //로드가 완료된 후에 세팅 
                $scope.gnbMenuOpen = function(flag){              
                    $scope.open_menu = flag;  
                    if(flag){
		...
		function getRootMenu() {
			...
			.success(function (data) {
				...
				// 20180419 초기 랭딩시 탭 GA 이벤트 태깅
				// 20180703 홈버튼 클릭 (tclick code 활용) 여부 체크
				var tclickChk =  commInitData.query["tclick"],
					GAchk = (tclickChk && (tclickChk.indexOf("logo") > -1 || tclickChk.indexOf("home") > -1)) ? false : true;
				displayMenuList.forEach(function(list){

					if(GAchk){
						if(list.dispNo == rendingTabDispNo ){
							$scope.logGAEvtComm("cateDirect_pv",list.dispNm);
							$scope.pageUI.cateDirectYn = true;
						}else{
							if(list.subTab && list.subTab.total_count > 1 ){ // 서브탭 확인
								for(var chk = 0; chk < list.subTab.total_count; chk++){
									if(list.subTab.items[chk].dispNo == rendingTabDispNo){
										$scope.logGAEvtComm("cateDirect_pv",list.dispNm);
										$scope.pageUI.cateDirectYn = true;
									}
								}
	...
	app.directive('lotteHeaderMain', ['$timeout', '$window', '$location', 'LotteCommon', 'LotteCookie', 'LotteStorage', 'AppDownBnrService', '$http',
		function ($timeout, $window, $location, LotteCommon, LotteCookie, LotteStorage, AppDownBnrService, $http) {
		return {
			templateUrl : "/lotte/resources_dev/main/lotte_header_main_2018.html",
			replace : true,
			...
			link : function ($scope, el, attrs) {
				...
				// 헤더 상단 메뉴 클릭 (GNB)
				$scope.headerMenuClick = function (dispNo, sub) {
					if($scope.ajaxLoadFlag) {
						return false;
					}
					...
					var menuInfo = $scope.getHeaderMenuInfo(dispNo);
					if($scope.pageUI.rootMenu[menuInfo.idx].subTab) {                        						
                        if(sub){ //추가된 소스  오픈 메뉴용 
                            $scope.pageUI.rootMenu[menuInfo.idx].tabIndex = sub.tabidx;
                            $scope.loadData(0, sub.dispNo, "contentContainer");
                        }else{ //기존
				...
				// 메인헤더 롯데슈퍼 링크
				var spUrl = "";
				// var spMainJsonUrl = "/json/main_new/getAppHeaderIcon.json";
				...
				.success(function (data) {
					$scope.data = data;
					if($scope.data.header_icon.main && $scope.data.header_icon.main.imgUrl){
						$scope.site_move = $scope.data.header_icon.main.imgUrl;
						spUrl = $scope.data.header_icon.main.linkUrl;
					} else {
						$scope.site_move = "http://image.lotte.com/lotte/mo2018/common/Tab_SuperLogo.gif";
						spUrl = "http://m.lottesuper.co.kr";
					}
				}).error(function(data) {
					$scope.site_move = "http://image.lotte.com/lotte/mo2018/common/Tab_SuperLogo.gif";
					spUrl = "http://m.lottesuper.co.kr";
				...
				// 슈퍼 메인 이동
				$scope.superMainMove = function () {
					var mainCHLNO = ($scope.appObj.isSuperApp) ? "M389194" : "M389182"; // 슈퍼 base : 닷컴 base
					if(!$scope.appObj.isApp){ mainCHLNO = "M389467"; }
					$scope.gotoLotteSuperUrl(spUrl, mainCHLNO); // params (연결URL, 채널번호)
				};
			}
		}
	}]);
	...
	app.filter('radnomid', function(){
		return function (txt) {
			return txt + "_" + Math.floor(Math.random()*100000);
		}
	});    
	// Main Menu
