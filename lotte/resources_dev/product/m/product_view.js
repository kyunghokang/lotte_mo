...
function searchMartResult(paramObj){
    ...
    var radio_name = "#mart_b" + martIndex;
    if(paramObj.crspk_corp_cd == '3000'){
        radio_name = "#mart_c" + martIndex;
    }
    $(radio_name).trigger("click");
...
(function(window, angular, undefined) {
	...
	// 전체 Controller
    /**/
	app.controller('ProductCtrl', ['$scope', '$window', '$http', '$timeout', '$interval', '$routeParams', '$location', 'LotteCommon', 'Fnproductview', 'commInitData', 'LotteUtil',"StyleRecom", "LotteCookie",
		function ($scope, $window, $http, $timeout, $interval, $routeParams, $location, LotteCommon, Fnproductview, commInitData, LotteUtil,StyleRecom, LotteCookie) {
    /*
    ...
    */
		...
		$scope.swipeTclick = "m_DC_ProdDetail_"; // v-com : 동영상 버튼 티클릭 추가
		$scope.popTclick = "m_DC_ProdDetail_List_"; // v-com : 동영상 버튼 티클릭 추가
		$scope.showStyleFlag = true; // v-com : 스타일샵 플래그 감추기 여부
		...
		*/
		...
		//20170824 AB 배너 영역 체크 
        if(todayTime > strToTime("20170900")){
           $scope.AB_TRACK_NO = 100;
        }
		...
		if (!Fnproductview.isEmpty(commInitData.query['goods_no'])) {
			...
			$scope.op_goods_no = $scope.reqParam.goods_no; //goods_no 를 유지하기위함 201701
		}
		...
		$scope.calcPromInfoOrg = {}; // 미리계산용 청구할인 원본 데이타
		$scope.calcPromInfo = {}; // 미리계산용 청구할인 데이타
		...
		var getPriceInfo = function () {
			...
			if ($scope.BasicData.product.org_sale_prc != $scope.priceInfo.lowPrice) { // 오리지날 가격이 할인 가격과 같지 않을 경우
		...
		function animateStyleIcon(){
			...
			if(n >= 0){
				var x = -60 * (n % 10);
				var y = n < 10 ? 0 : -60;
				angular.element(".recomm_style2 .re_style_btn span").css("background-position", x+"px " + y+"px");
			}
		};
        //20170413 배송비영역 무료배송 출력 여부 
		...
		// v-com : 동영상 호출 함수
		$scope.autoVdoPlay = function(){
			if($scope.BasicData.product.movies != undefined && $scope.BasicData.product.movies[0].mpic_url_addr != undefined && $scope.BasicData.product != ""){
				// 동영상이 있으면 초기화
				$timeout(function(){
		...
		$scope.loadBasicData = function() {
			if ($scope.BasicData == undefined) {
				...
				.success(function(data) {
                    ...
                    $scope.styleFlag = $scope.BasicData.product.style_flag;
                    if($scope.styleFlag == undefined || $scope.styleFlag == null){
                        $scope.styleFlag = false;
                    }
                    if($scope.styleFlag){
                    	$interval(animateStyleIcon, 100);
                    }
                    $scope.recommStylePop = false; //스타일추천 팝업 
                    var STYLE_LINK = "/product/m/style_recom.do";
                    $scope.rs_open = localStorage.getItem('recommstyle'); //팝업 리스트 가능                 
                    $scope.rs_msg = false; // v-com : 메시지창                                                                    
                    if(todayDate != localStorage.getItem('recommstyle_day')){
                        $scope.rs_msg = true;
                    }
					// v-com : 영상상품평 알람 레이어
					$scope.ms_msg = false; // 메시지창
					if(todayDate != localStorage.getItem('recommMovie_day')){
                        $scope.ms_msg = true;
                    }
					// v-com : 영상상품평 닫기 버튼
					$scope.close_moviemsg = function(){
                        $scope.ms_msg = false;
                        localStorage.setItem('recommMovie_day', todayDate);
                    };
                    if($scope.rs_open == null){$scope.rs_open = "Y"};                                                        
                    $scope.imglink = $scope.BasicData.product.img_url0.replace('_280', '_550');                    
                    $scope.nostyle = false;                    
                    $scope.data_styleRecomm = function(){
                        $scope.sendTclick("m_DC_ProdDetail_Clk_Pop3");
                        if($scope.imageList.length > 1){
                            var image_no = parseInt($(".currentNum").text()) - 1;
                            if($scope.imageList[image_no].tp=='vod'){ //20170313, 유튜브
                                image_no = 1;
                            }else if($scope.imageList[image_no].tp=='vde'){ // v-com : 이미지영역 동영상
                                image_no = 0;
                            }
                            $scope.imglink = $scope.imageList[image_no].imgUrl.replace('_280', '_550');  
                        }
                        StyleRecom.detect($scope.imglink, function(list){                            
                            if(list.length > 0){
                                if($scope.rs_open == 'Y'){
                                    localStorage.setItem('recommstyle', 'N');
                                    $scope.rs_open = 'N';
                                    //show list              
                                    var cate = list[0].category;
                                    $scope.selectedSex = 'A';//A, M, F
                                    if(cate == 1 || cate == 8){
                                        $scope.selectedSex = 'F';
                                    }else{
                                        //console.log("genSctCd : ", $scope.loginInfo.genSctCd);
                                        if($scope.loginInfo.isLogin && $scope.loginInfo.genSctCd){
                                            $scope.selectedSex = $scope.loginInfo.genSctCd;   
                                        }
                                    }
                                    StyleRecom.search(list[0].id, $scope.selectedSex, [cate], $scope.BasicData.product.styl_recomm_cat_grp_cd, function(data){                                
                                        $scope.style_list = data.slice(0, 5);
                                        //console.log($scope.style_list);
                                        if(data.length > 0){
                                            $(".recommgroup").show();
                                        }
                                    });                                                                                            
                                ...
                                //추천목록이 없는 경우이고, 이때 남들은 뭘샀지를 조회해서 보여줌
                                var tmphour = new Date().getHours();
                                if (("" + tmphour).length  == 1) {
                                    tmphour  = "0" + tmphour;
                                }
                                $scope.sendTclick("m_DC_ProdDetail_Error_" + tmphour + "_" + $scope.BasicData.goods_no);                                
                                $scope.nostyle = true;     
                                var wst = $(window).scrollTop();
                                if(SaleBestDataCopy != ""){
                                    $scope.style_list = makeStyleData();                                    
                                    $(".recommgroup").show();           
                                    $(window).scrollTop(wst + 1);
                                }else{
                                    $('body').animate({"scrollTop":wst + 100}, 300);
                                    $timeout(function(){
                                        $scope.style_list = makeStyleData();
                                        if($scope.style_list.length == 0){
                    ...
                    function makeStyleData(){ //남들은뭘샀지를 스타일데이타로 가공
                        var tmpar = [];
                        if(SaleBestDataCopy != ""){                            
                            var data = SaleBestDataCopy;
                            for(var i=0; i<data.length && i < 5;i++){
                                tmpar.push({
                                    goodsNo : data[i].goods_no ,
                                    imgUrl: data[i].img_url,
                                    price2 : data[i].sale_prc - data[i].inst_cpn_aply_unit_price - data[i].immed_pay_dscnt_amt
                                });
                            }
                        }
                        return tmpar;                        
                    ...
                    //20170411                    
                    $scope.style_go = function(tclick){
                        var more = "";
                        if($scope.BasicData.product.styl_recomm_cat_grp_cd != undefined){
                            more = "&cate=" + $scope.BasicData.product.styl_recomm_cat_grp_cd;
                        }
                        var tclickCode = "&tclick=m_DC_ProdDetail_Clk_Lnk_3";
                        if(tclick){
                            tclickCode = "&tclick=" + tclick;
                        }
                        location.href = $scope.baseLink(STYLE_LINK + "?img=" + $scope.imglink) + more + tclickCode;
                    }
                    
                    $scope.style_gotoProduct = function(goodno, index){
                        location.href = $scope.baseLink(LotteCommon.prdviewUrl) + "&goods_no=" + goodno +"&tclick=m_DC_ProdDetail_Clk_Rel"+ index;
                    }
                    $scope.close_stylemsg = function(){
                        //$('.re_style_msg').hide();
                        $scope.rs_msg = false;
                        localStorage.setItem('recommstyle_day', todayDate);
                    }
                    $scope.close_stylepop = function(){
                        $('.recommgroup').hide();
                        $scope.rs_msg = false;
                        //$scope.close_stylemsg();
                    }
                    //스타일추천 20170223 ========================= end 
					
					/* 샤넬 컬러리스트 */	
					if($scope.BasicData.chanel_yn == true){						
						
						if($(window).width() < 640){								
							/* 샤넬 관련 상품 */
					    	var chanelRelDataTotal = $scope.BasicData.product.chanel_rel_product_list.items != null ? $scope.BasicData.product.chanel_rel_product_list.items.length : 0;
					    	var chanelRelSliceData = [];
	
					    	if ($scope.BasicData.product.chanel_rel_product_list.items && chanelRelDataTotal > 0) {
					    		var i = 0,
					    			j = -1;
	
					    		for (i; i < chanelRelDataTotal; i++) {
					    			if (i % 2 == 0) {
					    				j++;
					    				chanelRelSliceData[j] = [];
					    			}
					    			chanelRelSliceData[j].push($scope.BasicData.product.chanel_rel_product_list.items[i]);
					    		}
					    		$scope.chanelRelProductData = chanelRelSliceData;
					    	}else{
					    		$scope.chanelRelProductData = null;
					    	}
					    	
					    	/* 샤넬 베스트 상품 */
					    	var chanelBestDataTotal = $scope.BasicData.product.chanel_best_seller_prod_list.items != null ? $scope.BasicData.product.chanel_best_seller_prod_list.items.length : 0;
					    	var chanelBestSliceData = [];
	
					    	if ($scope.BasicData.product.chanel_best_seller_prod_list.items && chanelBestDataTotal > 0) {
					    		var i = 0,
					    			j = -1;
	
					    		for (i; i < chanelBestDataTotal; i++) {
					    			if (i % 2 == 0) {
					    				j++;
					    				chanelBestSliceData[j] = [];
					    			}
					    			chanelBestSliceData[j].push($scope.BasicData.product.chanel_best_seller_prod_list.items[i]);
					    		}
					    		$scope.chanelBestProductData = chanelBestSliceData;
					    	}else{
					    		$scope.chanelBestProductData = null;
					    	}
						}											
					...
					// v-com : 상단 이미지영역 동영상 정의
					if ($scope.BasicData.product.movies != undefined && $scope.BasicData.product.movies[0] != undefined && $scope.BasicData.product.movies[0].mpic_url_addr != null && $scope.BasicData.product.movies[0].mpic_url_addr != '') {
						var item = {};
						item.tp = "vde";
						item.img_url_addr = $scope.BasicData.product.movies[0].img_url_addr;
						item.mpic_url_addr = $scope.BasicData.product.movies[0].mpic_url_addr;
						
						$scope.imageList.push(item);
						$scope.BasicData.product.vod_url=''
					};
                    ...
                    // v-com : 상단 이미지 영역 - 스와이프 동작
					$scope.swipeEndHandler = function(event){
						// 동영상이 있는 화면인 경우
						if($scope.BasicData.product.movies && $scope.BasicData.product.movies[0].mpic_url_addr){
							$scope.showStyleFlag = true;
							$scope.swipeVideoStop(); // 동영상 정지
							var scope = angular.element('.swipePlayBtn .playBtn').scope();
							var current = event.idx + 1; // 현재 번호
							// 마지막 슬라이드면
							if (current == scope.imageList.length){
								vod_useScroll = true; // 스크롤 자동재생 기능 활성화
								$scope.autoVdoPlay(); // 동영상 호출 함수
								$scope.showStyleFlag = false; // 스타일샵 플래그 감추기
							}else{
					...
					if($scope.BasicData.product.clm_dscnt_prom_items && $scope.BasicData.product.clm_dscnt_prom_items.length > 0){
						...
						for(var i = 0; i<allItems.length; i++){
							...
							if(tempItem.aply_lmt_amt <= $scope.priceInfo.lowPrice){
								$scope.isMatchLmt = true;
							}
							$scope.dispPromInfo_base.push(tempItem);
						}
						if($scope.BasicData.product.select_goods_list.total_count > 0){// 기획전형
	                    	if($scope.BasicData.product.clm_dscnt_prom_card_items && $scope.BasicData.product.clm_dscnt_prom_card_items.length > 0){
								var items = $scope.BasicData.product.clm_dscnt_prom_card_items;
								var objPlanTemp = [];
								$scope.isMatchLmt = false; // 초기화
								for(var i = 0; i<items.length; i++){
									var item = {};
									item.card_knd_nm = items[i][0];
									var objItem = $scope.objPromDisc.getMatchItem(item, $scope.dispPromInfo_base);
									if(!objItem)	continue;
									item = objItem;
									if(item.aply_lmt_amt <= $scope.priceInfo.lowPrice){
										$scope.isMatchLmt = true;
									}
									objPlanTemp.push(item);
								}
								// 구간대 부합하는 것이 하나도 없으면
								var cnt = 0;
								if(!$scope.isMatchLmt){
									for(var i = 0; i<objPlanTemp.length; i++){
										var tempItem = objPlanTemp[i];
									...
									for(var i = 0; i<objPlanTemp.length; i++){
										var item = objPlanTemp[i];
										// 처음나온 카드이거나 부합하는 구간이면서 최종금액이 작은경우에 데이타 갱신
										if(!$scope.dispPromInfo_before[item.card_knd_cd] || (item.aply_lmt_amt <= $scope.priceInfo.lowPrice && item.prom_dscnt_prc < $scope.dispPromInfo_before[item.card_knd_cd].prom_dscnt_prc)){
											item.sort = cnt++;
            		...
            		$timeout(function(){
	 				...
	 				function aceRetargetingDetail(){
	 					if($('#aceRetargetingDetail').length == 0){
	 						...
	 						scr.onload = function(){
	 							...
	 							window.ne_tgm_q.push({
	 								...
	 								pageEncoding:'utf-8',
	 								items : [{
	 							        id: $scope.BasicData.goods_no/*ID값입력*/,
	 							        price:$scope.priceInfo.lowPrice/*가격정보입력*/,
	 							        quantity:"1"/*갯수정보입력*/,
	 							        category:""/*카테고리 대|중|소*/,
	 							        imgUrl:$scope.BasicData.product.img_url0/*img링크값입력 ex)http://example.com/img/img.jpg*/,
	 							        name:$scope.BasicData.product.goods_nm/*제품명또는 전환메뉴명입력*/,
	 							        desc:""/*상품상세 설명 text*/,
	 							        link:"http://m.lotte.com/product/m/product_view.do?goods_no=" + $scope.BasicData.goods_no/*상품상세페이지 URL ex)http://example.com/detail/product.html*/
	 							    }]
	 							});
	 						}
		...
		$scope.objPromDisc = {
			...
			// 기획전형인경우 전체에서 찾기
			getMatchItem : function(item, total){
				var result;
				for(var i = 0; i<total.length; i++){
					// 다른 카드면 Pass
					if(total[i].card_knd_nm != item.card_knd_nm)	continue;
					// 처음나온 카드이거나 부합하는 구간이면서 최종금액이 작은경우에 데이타 갱신
					if(!result || (total[i].aply_lmt_amt <= $scope.priceInfo.lowPrice && total[i].prom_dscnt_prc < result.prom_dscnt_prc)){
						result = total[i];
					}
				}
				return result;
			...
			getDiscPrice : function(item){
				...
				// 할인가격
				var disc = $scope.priceInfo.lowPrice * item.fvr_val * 0.01;
				// 할인가격과 최대할인 가격 비교
				if(item.max_fvr_val < disc){
					return $scope.priceInfo.lowPrice - item.max_fvr_val;
				}
				return $scope.priceInfo.lowPrice - disc;
			},
			// 구간대에 부합하는 경우 차상위 찾기
		...
		// v-com : 상단 이미지 영역 재생버튼 티클릭
		$scope.movieSwipeTclick = function(tclick, tclick2){
			tclick = $scope.swipeTclick + tclick;
			if ($(".dSwipeImg div.video_wrap > a.btn_move_volume").hasClass("mute")){
				tclick = $scope.swipeTclick + tclick2;
			}
			$scope.sendTclick(tclick);
		};

		// v-com : 상품설명 - 묶음상품 클릭시 레이어팝업 재생버튼 티클릭
		$scope.moviePopTclick = function(tclick, tclick2){
			tclick = $scope.popTclick + tclick;
			if ($(".lPrdTop div.video_wrap > a.btn_move_volume").hasClass("mute")){
				tclick = $scope.popTclick + tclick2;
			}
			$scope.sendTclick(tclick);
		};
	...
	// 하단 주문레이어 시작
	app.directive('footerOptionbar', ['$http', '$window', '$timeout', '$location', '$compile', 'LotteCommon', 'Fnproductview', 'LotteForm', 'LotteCookie', 'LotteLink','commInitData',
		function($http, $window, $timeout, $location, $compile, LotteCommon, Fnproductview, LotteForm, LotteCookie, LotteLink,commInitData) {
		return {
			...
			link: function ($scope, el, attrs) {
						...
						if ($scope.BasicData.goods_cmps_cd == '30') {
							...
							*/
							if ($scope.BasicData.product.item_dtl == null) {
								...
								$scope.addOrderList(getOrderListItem(null, $scope.BasicData.goods_no, null, null, $scope.BasicData.product.item_no, $scope.smpDispYn
				...
				var getOrderListItem = function (goods_nm, goods_no, item_nm, color_img_url, item_no, smp_yn
					...
					rtnItem.color_img_url = color_img_url;
					...
					if (pItem == null) {
                        ...
                        rtnItem.smp_psb_yn = $scope.BasicData.product.smp_psb_yn;
                        //rtnItem.gift_pkg_yn = "";
					} else {
                        ...
                        rtnItem.smp_psb_yn = pItem.smp_psb_yn; //스마트픽동선개선
						...
						if ($scope.goodsOptData.item_dtl.total_count > 0) {
							for (var i = 0; i < $scope.goodsOptData.item_dtl.items[0].item.total_count; i++) {
								...
								if (item.item_no == item_key) {
									...
									rtnObj.color_img_url = item.color_img_url;
				...
				$scope.deliSelect_layer = function (v) {
					...
					}else if(v == '2'){ //20170817 스마트픽 안내 팝업
                        $("body").addClass("fixBody");
                        $scope.inputNumber=true;//팝업 열때
                        $scope.smartPop = true;                        
                    }
                ...
                $scope.btnOrderNormal_click = function(type){ 
                    ...
                    $scope.btnOrder_click(type);                    
                }
                $scope.btnOrder_click = function(type){
                    //모바일상품권 구매 대상 제어 20170223                    
                    pticketCheck(0, commInitData.query["goods_no"], function(flag){
                       if(flag){
                           return;
                       }else{                
                           $scope.btnOrder_click2(type);                    
                       } 
                    });                    
                }
				// 바로주문f
				$scope.btnOrder_click2 = function (type) {
					if($scope.orderFlag){
						if($scope.orderTypeGift){
							$scope.sendTclick( 'm_DC_ProdDetail_Gift_Main_btn' );
						}else if(type == 'normal'){
							$scope.sendTclick( 'm_DC_ProdDetail_Clk_Btn_buy' );
						}else{
                ...
                $scope.btnCart_click = function(){
                    ...
                    pticketCheck(0, commInitData.query["goods_no"], function(flag){
                       if(flag){
                           ...
                           $scope.btnCart_click2();
                       } 
                    });                    
                }
                
				$scope.btnCart_click2 = function () {
                    ...
                    if ($(".layerPop.detailInfoPop").length > 0 && nextOptFlag) {
                        ...
                        /**
                		 * BUZZNI 리타겟팅 스크립트 (Basket)
                		 * 2017.03.24
                		 */
                        if($scope.sendBuzzni)	$scope.sendBuzzni("cart", [commInitData.query["goods_no"]]);
                    }
						...
						*/
						if ($scope.goodsOptData.item_dtl.items == null || $scope.goodsOptData.item_dtl.items[0].item.items[0].opt_value == null) {//기획전형 상품 중 스마트픽 가능한 상품 선택 시
							if (!$scope.deliSelect) { // 스마트픽상품은 지점 선택시 자동으로 넣어 준다.
								...
								$scope.addOrderList(getOrderListItem($scope.goodsOptData.goods_nm, goods_no, null, null, $scope.BasicData.product.item_no, $scope.smpDispYn
				...
				var addOrderItemProc = function (b, p_inv_qty) {
					...
					if (isAllSelected) {
						...
						if (b) {
							return getOrderListItem(goods_nm,goods_no, optObj.opt_tval, optObj.color_img_url, optObj.item_no, smpDispYn
							...
							$scope.addOrderList(getOrderListItem(goods_nm, goods_no, optObj.opt_tval, optObj.color_img_url, optObj.item_no, smpDispYn
								,ord_cnt, $scope.priceInfo.lowPrice, inv_qty, master_disp_no, goods_cmps_cd
								,smartOrd, img_url, $scope.BasicData.min_lmt_qty, $scope.BasicData.max_lmt_qty, smp_entr_no
				...
				$scope.selPrdDel_click =  function (goods_no, item_no) {
                    ...
                    $scope.check_enableSmartpick();
					checkQuick();
					$scope.addOrderList(null); // null bind이면 주문항목을 추가하지 않고 수량과 금액만 재계산한다.
				};
                
                //백화점과 크로스픽이 엇갈려 있는지 체크 : 스마트픽동선개선
                $scope.check_enableSmartpick = function(){
                    if($scope.totalDelivery_type == "0"){ //택배 또는 스마트픽 겸용일 때에만 체크
                        var onlygoods = [0, 0];
                        for (var i = 0; i < $scope.orderList.length; i++) {
                            if ($scope.orderList[i].crspk_psb_yn == "N" && $scope.orderList[i].smp_psb_yn == "Y") {
                                onlygoods[0] += 1;    
                            }else if($scope.orderList[i].crspk_psb_yn == "Y" && $scope.orderList[i].smp_psb_yn == "N"){
                                onlygoods[1] += 1;    
                            }else if($scope.orderList[i].crspk_psb_yn == "N" && $scope.orderList[i].smp_psb_yn == "N"){
                                $scope.totalDelivery_type = "1";         
                                break;
                            }
                        }
                        if(onlygoods[0] > 0 && onlygoods[1] > 0){
                            $scope.totalDelivery_type = "1";         
                            //$scope.apply();
                        }
                    }
                    //적용이 안되는 케이스가 있어서 강제적용
                    if($scope.totalDelivery_type == "1"){
                        $('#method9').prop('checked',true);
                        $scope.qs_yn = 'N';
                    }else if($scope.totalDelivery_type == "2"){
                        $('#method10').prop('checked',true);
                        $scope.qs_yn = 'N';
                    }
                }
...
//						$scope.imall_prod_chk('buy'); // 아이몰 재고수량 체크
						...
						if (!$scope.loginInfo.isLogin) {  // 로그인 안한 경우 !$scope.loginInfo.isLogin eung 임시 로 막음
							...
							if ($scope.BasicData.smp_wine_yn == 'Y') { // 와인 상품이면
								...
								if (isSmp) { // 스마트픽 예약주문 이거나 크로스픽인 경우 
									...
									goUrl = LotteCommon.secureUrl + "/order/m/order_form.do?" + $scope.baseParam;
								}
							}
						}
                        
						goUrl += '&tclick=m_RDC_ProdDetail_Clk_smartpick';
				...
				function sendNSMLogging(){
					...
					// products
					len = $scope.orderList.length;
					for(var i=0; i<len; i++){
						prd = $scope.orderList[i];
						qty = prd.ord_cnt;
						tot = prd.sale_price;
						prc = prd.org_sale_price;
						pno = prd.goods_no + "_" + prd.goods_nm;
						etc = i;
				...
				var setOrderSendInfo = function () {
                    ...
                    //크로스픽 추가    
                    var crspk_yn = "",crspk_psb_yn = "", crspk_corp_cd = "",crspk_corp_str_sct_cd = "", crspk_str_no = "", crspk_store = "";
					...
					if ($scope.orderList != null) {
						...
						for(var i = 0; i < $scope.orderList.length; i++) {
							...
							if ( $scope.BasicData.smp_wine_yn == 'Y' 
                                 || ( !$(".selList > li").eq(i).hasClass("disable") 
                                        && !(item.delivery_type != undefined && item.delivery_type != '1' 
                                        && item.smp_shop_no == '' && $(".radio01:checked").val() == 2)) 
                                 || $("#method10:checked").length > 0
                                 //smartpick_new || (item.crspk_corp_cd != undefined && item.crspk_corp_cd != '')
                               ) {
								...
								select_std_goods_no += gbn_str + $scope.op_goods_no;//$scope.reqParam.goods_no;//item.goods_no; 20170124
                                ...
                                crspk_psb_yn += gbn_str + undefinedCheck(item.crspk_psb_yn);
                    ...
                    orderSendInfo.crspk_psb_yn = crspk_psb_yn;
                ...
                //201605 크로스픽
                //지도검색 20161130
                $scope.show_map = function(radioIndex, param, corp_cd){
                    martIndex = radioIndex;          
                    //초기화
                    if($scope.smpData.goods_list.items[radioIndex].crspk_corp_cd != corp_cd){
                        $scope.smpData.goods_list.items[radioIndex].crspk_corp_cd = corp_cd;                    
                        $scope.smpData.goods_list.items[radioIndex].mart_name = "";
                    }
                    
                    $("#searchMartPop > iframe").attr("src", $scope.google_map_url + '?store_div=' + param);
                    $("#searchMartPop").show();                    
                }
				...
				//20170817 스마트픽 안내 팝업
				$scope.close_smartPop = function(){
					$scope.inputNumber = false;
					$scope.smartPop = false;
					$("body").removeClass("fixBody");
				}
			}
		};
	...
	// Directive :: 상품상세 상단기본정보[샤넬] 셋팅
	app.directive('productInfoChanel', ['Fnproductview', '$window', 'LotteCommon', '$timeout',  function (Fnproductview, $window, LotteCommon, $timeout) {
		return {
			templateUrl: '/lotte/resources_dev/product/m/product_info_chanel.html',
			link: function ($scope, el, attrs) {
				//console.log('productInfoWine called');
				$scope.Fnproductview = Fnproductview;					
				if ($scope.BasicData.product.colors.items.length > 0){
					$scope.ColorNo = ""; //컬러넘버값							
					$scope.ColorClick = function(item, tclick, index) {
	            		if($scope.ColorNo == item.color_no) {
	        				$scope.ColorNo = [];
	        			} else {
	            			$scope.ColorNo = item.color_no;
	        			}
	            		$scope.sendTclick( 'm_DC_ProdDetail_Swp_Color_idx0' + ( index + 1 ) );
	            	}						
					//$scope.ColorClick($scope.BasicData.product.colors.items[0]);					
					
					/* 샤넬 색상 */
			    	var chanelColorDataTotal = $scope.BasicData.product.colors.items != null ? $scope.BasicData.product.colors.items.length : 0;
			    	var chanelColorSliceData = [];

			    	if ($scope.BasicData.product.colors.items && chanelColorDataTotal > 0) {
			    		var i = 0,
			    			j = -1;

			    		for (i; i < chanelColorDataTotal; i++) {
			    			if (i % 6 == 0) {
			    				j++;
			    				chanelColorSliceData[j] = [];
			    			}
			    			chanelColorSliceData[j].push($scope.BasicData.product.colors.items[i]);
			    		}
			    		$scope.chanelColorData = chanelColorSliceData;
			    	}else{
			    		$scope.chanelColorData = null;
			    	}
				}
			}
		};
	...
	app.directive('popPreCalculator', ['$window', '$http', '$timeout', 'LotteCommon', 'Fnproductview',
		...
		return {
			...
			link: function ($scope, el, attrs) {
				...
				$scope.usePriceInfo.showProm = false; // 청구할인 select 노출여부
									...
									for (var i = 0; i < tmp_prom.length; i++) {
										...
										if (tmp_prom2[0] == first_cpn && tmp_prom2[1] == val_arr[2]) {
											...
											if (dup_amt < 1) { // 중복쿠폰 금액이 0이면 비활성화
												...
												$scope.alert_2016("죄송합니다. 추가할인 불가능 상품입니다.");
												$scope.usePriceInfo.cbDupDiscOption = "";
				...
				// 청구할인 select 변경
				$scope.changeProm = function(){
					$scope.usePriceInfo.card_knd_nm = angular.element('#cbPromOption option:selected').text();
					var arr = $scope.usePriceInfo.cbPromOption.split('|');
					if(arr.length > 1){
						$scope.usePriceInfo.promAmt = $scope.usePriceInfo.ordAmt - ($scope.usePriceInfo.ordAmt * arr[0] * 0.01);
					}
				};
				// 미리계산에서 제한금액 부합 청구할인 구하기
				$scope.setCalcPromInfoOrg = function(){
					$scope.usePriceInfo.cbPromOption = '';
					$scope.calcPromInfoOrg = {};
					var total = $scope.BasicData.product.clm_dscnt_prom_items;
					for(var i = 0; i<total.length; i++){
						var item = total[i];
						// 제한금액에 걸리면 해당안됨
						if(item.aply_lmt_amt > $scope.usePriceInfo.ordAmt){
							continue;
						}
						// 기존에 것이 있으면 할인율을 비교한다.
						if($scope.calcPromInfoOrg[item.card_knd_cd] && $scope.calcPromInfoOrg[item.card_knd_cd].fvr_val >= item.fvr_val){
							continue;
						}
						$scope.calcPromInfoOrg[item.card_knd_cd] = item;
					}
					// 카드에 부합한 청구할인 구하기
					$scope.calcPromInfo = {};
					// 할인카드가 없으면 모두 노출
					if(!$scope.usePriceInfo.cbDiscOption){
						$scope.calcPromInfo = $scope.calcPromInfoOrg;
					}else{
						var arr = $scope.usePriceInfo.cbDiscOption.split("|");
						$scope.usePriceInfo.curCardCd = arr[arr.length-1];
						// 카드코드가 없는 경우이면
						if(!$scope.usePriceInfo.curCardCd){
							$scope.calcPromInfo = $scope.calcPromInfoOrg;
						}else{
							for(var key in $scope.calcPromInfoOrg){
								if(key === $scope.usePriceInfo.curCardCd){
									$scope.calcPromInfo[key] = $scope.calcPromInfoOrg[key];
								}
							}
						}
					}
					if(Object.keys($scope.calcPromInfo).length > 0){
						$scope.usePriceInfo.showProm = true;
					}else{
						$scope.usePriceInfo.showProm = false;
					}
					// 금액계산 표시
					var arr = $scope.usePriceInfo.cbPromOption.split('|');
					if(arr.length > 1){
	...
	////////////////////////////////////////////////////////
	// 전체 Controller
	app.controller('ProductDetailCtrl', ['$scope','$window', '$http', '$routeParams', '$location', 'LotteCommon','Fnproductview','commInitData','LotteUtil','LotteStorage','StyleRecom',function($scope,$window, $http,$routeParams, $location, LotteCommon,Fnproductview,commInitData,LotteUtil,LotteStorage,StyleRecom) {
            ...
            if(LotteStorage.getSessionStorage("qnaBack") == "true"){
				...
				setTimeout(function () {
                    ...
                    $scope.show_tprdlist = false;
                    if($scope.BasicData.product.select_goods_list.items != null){
                        $scope.select_product = $scope.BasicData.product.select_goods_list.items[LotteStorage.getSessionStorage("qnaindex")];
                        $scope.reqDetailParam.usm_goods_no = $scope.select_product.usm_goods_no;
                    }
                    $scope.getFaqListPaging(0,$scope.selectedItem,'S');
                    
                    //$scope.getFaqListPaging(0, LotteStorage.getSessionStorage("qnaguest"), "S");
                    //$scope.loadingTabData(2);
                    //화면 스크롤 
                    $('body').animate({"scrollTop": ($(".detailInfoCnt").offset().top - 85)+"px"}, 500);
			...
			$scope.loadingTabData = function (idx) {
				...
				if (idx < 4) {
                    ...
                    //console.log($scope.productInfotabUrl);
                     //201701 상품평개선
                     if($scope.BasicData.product.select_goods_list.items != null && $scope.BasicData.goods_cmps_cd == '30' && idx > 0){                         
                         $scope.select_product = $scope.BasicData.product.select_goods_list.items[0];
                         $scope.reqDetailParam.usm_goods_no = $scope.select_product.usm_goods_no;
                         $scope.select_product.index = 1;
                         $scope.reqDetailParam.goods_no = $scope.select_product.goods_no; 
                         $scope.reqDetailParam.star = 0; //별점 전체를 기본값
                         $scope.reqDetailParam.sort = "L"; //sort 기본값
                     }
                     
					if ($scope.BasicData.disp_yn) { // 전시/비전시 여부 파라메터 전시에 넘겨주기 위한 파라메터 추가
						$scope.reqDetailParam.disp_yn = $scope.BasicData.disp_yn;
					}
                ...
                $scope.show_tprdlist = false;
                $scope.sort_tab_open = false;
                $scope.sort_tab_open2 = false;
                $scope.sort_tab = true; //true 정렬, false : 별점
                $scope.changeSorttab = function(flag){ //탭변경하기
                    if(flag){ //정렬순                       
                        if(flag != $scope.sort_tab){                        
                            $scope.sort_tab_open = true;
                            $scope.sort_tab_open2 = false;
                        }else{
                            $scope.sort_tab_open = !$scope.sort_tab_open;     
                        }                        
                    }else{ //별점 종류별                        
                        if(flag != $scope.sort_tab){                        
                            $scope.sort_tab_open2 = true;
                            $scope.sort_tab_open = false;
                        }else{
                            $scope.sort_tab_open2 = !$scope.sort_tab_open2;     
                        }                        
                    }
                    $scope.sort_tab = flag;                    
                }
                $scope.toggleSelect = function(){
                    $scope.show_tprdlist = !$scope.show_tprdlist;
                }
                $scope.gotoPhotoReview = function(gdas_no, index){                   
                    if($scope.BasicData.product.select_goods_list.items != null && $scope.BasicData.goods_cmps_cd == '30'){
                        location.href = LotteCommon.photoreviewPage + "?usm_goods_no="+$scope.select_product.usm_goods_no+"&goods_no=" +$scope.select_product.goods_no+ "&id=" + index 
                            + "&" + $scope.baseParam  + "&tclick=m_DC_menu_5553646_Swp_Prd_idx0"+(index + 1)+"_sort_F_40&goods_img=" +  $scope.share_img + "&totCnt=" + $scope.PrdCommentData.photos.photos_more; 
                    }else{
                        location.href = LotteCommon.photoreviewPage + "?usm_goods_no="+$scope.BasicData.product.usm_goods_no+"&goods_no=" +$scope.reqParam.goods_no+ "&id=" + index 
                            + "&" + $scope.baseParam  + "&tclick=m_DC_menu_5553646_Swp_Prd_idx0"+(index + 1)+"_sort_F_40&goods_img=" +  $scope.share_img + "&totCnt=" + $scope.PrdCommentData.photos.photos_more;                         
                    }
                }
                //상품평 셀렉트박스 선택시 
                $scope.selectedProduct = function(item, index){
                    $scope.show_tprdlist = false;
                    $scope.select_product = item;
                    $scope.reqDetailParam.usm_goods_no = $scope.select_product.usm_goods_no;
                    $scope.select_product.index = index;                    
                    $scope.getCommentViewListPaging(0,'S',item.goods_no);
                }
                //QnA 셀렉트박스 선택시 
                $scope.selectedProduct2 = function(item, index){
                    $scope.show_tprdlist = false;
                    $scope.select_product = item;
                    $scope.select_product.index = index;
                    $scope.reqDetailParam.usm_goods_no = $scope.select_product.usm_goods_no;
                    //console.log(item.goods_no);
                    $scope.getFaqListPaging(0,item.goods_no,'S');
                }
                //구매정보 셀렉트박스 선택시 
                $scope.selectedProduct3 = function(item, index){
                    $scope.show_tprdlist = false;
                    $scope.select_product = item;
                    $scope.select_product.index = index;
                    $scope.productTabChange(3,item.goods_no);
                }
                // 201701 상품평개선 ================= end
                
                
				// 구매정보 일경우 데이터만 보여주고 있는 데이터 보이는 화면 처리만 ajax는 필요 없음
				if($scope.BasicData.chanel_yn == false){
					if ($scope.SaleBestData == "") {
						$scope.func_SaleBestData();
                        //함께사볼까 20170706
                        setTimeout(function(){
                            $scope.func_buyTogether();
                        }, 1000);
	                    //추천기획전 20161027
	                    setTimeout(function(){
            ...
            //20170420 하단에 스타일추천 리스트
            $scope.style_link = function(item, index){ //20170803 스타일추천 링크 이동
                if(index < 10){
                    index = "0" + index;
                }
                location.href = "/product/m/product_view.do?goods_no=" + item.goods_no + "&" + $scope.baseParam + "&curDispNoSctCd=14&tclick=m_DC_ProdDetail_Swp_Rel_D" + index;
            }
            $scope.foot_styleList = [];
            $scope.func_styleRecom = function(){                
                var image_no = 0;
                if($scope.imageList.length > 1){
                    image_no = parseInt($(".currentNum").text()) - 1;
                    if($scope.imageList[image_no].tp=='vod'){ //20170313
                        image_no = 1;
                    }
                }
                var imglink = $scope.imageList[image_no].imgUrl.replace('_280', '_550');                  
                StyleRecom.detect(imglink, function(list){
                    if(list.length > 0){
                        //show list              
                        var cate = list[0].category;
                        var selectedSex = 'A';//A, M, F
                        if(cate == 1 || cate == 8){
                            selectedSex = 'F';
                        }else{
                            if($scope.loginInfo.isLogin && $scope.loginInfo.genSctCd){
                                selectedSex = $scope.loginInfo.genSctCd;   
                            }
                        }
                        StyleRecom.search(list[0].id, selectedSex, [cate], $scope.BasicData.product.styl_recomm_cat_grp_cd, function(data){                                                            
                            var style_list = data.slice(0, 12); //20170803 갯수 변경
                            $scope.foot_styleList = [];
                            for(var i=0; i< style_list.length; i++){
                                //foot에 맞게 가공
                                $scope.foot_styleList.push({
                                    goods_no : style_list[i].goodsNo,
                                    img_url:style_list[i].imgUrl,
                                    goods_nm:style_list[i].brandNm +" "+style_list[i].goodsNm,
                                    sale_prc:style_list[i].price2
                                });
                            }
                        });                                                                                                                    
                    }
				...
				$scope.func_Tab0 = function(idx,data){
					...
					// md_ntc_fcont1: BO
					// md_ntc_fcont2: PO
                    //20170420 MD공지 해당 코드만 제거하는 거로 변경 : ['<p>&nbsp;</p>', '<p><br>&nbsp;</p>', '<p><br />&nbsp;</p>', '<br />', '<br>', 'null']                    
                    $scope.BasicData.product.md_ntc_fcont2 = $scope.BasicData.product.md_ntc_fcont2.replace(/\<br(.*?)\>|\<p\>\&nbsp\;\<\/p\>|\<p\>\<br(.*?)\>\&nbsp\;\<\/p\>|null/g, "");                    
					setTimeout(function(){
						$scope.BasicData.product.md_ntc_fcont1 = Fnproductview.replaceAll($scope.BasicData.product.md_ntc_fcont1, "&#34;", "'");
						$scope.BasicData.product.md_ntc_fcont1 = Fnproductview.replaceAll($scope.BasicData.product.md_ntc_fcont1, "\r\n", "");
                        //$scope.BasicData.product.md_ntc_fcont1  = $scope.BasicData.product.md_ntc_fcont1.replace(/\&\#34;/g, "'").(/\\r\\n/g, "");
					...
					if (data.result) {
                        ...
                        if($scope.BasicData.goods_cmps_cd == '50'){ //일반형 추가 20170621
                            $scope.scopeProductDetailDefault();
                        }else if($scope.PrdExplainData.spdp_shp_goods_tmpl_cd == "A"){
							$scope.PrdExplainData.goods_no_auto_set_yn='Y';
							$scope.PrdExplainData.goods_info_auto_prt_yn='Y';
					...
					$scope.scopeProductDetailDefault = function(){
						...
						//console.log("------------- default", $scope.lodomain.length, $scope.viewBigImg);
				...
				$scope.func_Tab1 = function(idx,data){
                        if(!data.data_set.product_review){
                            //201701 상품평개선    
                            data.data_set.stat = $scope.nullprObj();                              
                        }
				...
				$scope.func_Tab3 = function(idx,data){
						...
						if($scope.BasicData.product.select_goods_list.items != null){
                            //201701 상품평 개선
                            $scope.selectedProduct3($scope.BasicData.product.select_goods_list.items[0], 1);                            
						}
				};
                ...
                //20170706 보완재 추가 : 함께 사볼까
                $scope.buyTogether1 = [];
                $scope.buyTogether2 = [];
                $scope.func_buyTogether = function(callback) {
                        var recLink = "http://rb-rec-api-apne1.recobell.io/rec/a005?size=10&cps=false&format=jsonp&cuid=fdd29847-94cd-480d-a0d9-16144485d58b&iids="+$scope.reqDetailParam.goods_no;
                        $.ajax({
                                type: 'post'
                                , async: true
                                , url: recLink
                                ...
                                , success: function(data) {
                                        ...
                                        if(data.results != null && data.results.length > 0){
                                            ...
                                            $scope.reqDetailParam2 = {
                                                latest_prod : strr,
                                                isSearch : 'Y'
                                            }                                            
                                            $http.get("/json/main/main_latest_prod.json", {params:$scope.reqDetailParam2})
                                            .success(function(data) {	                                                    
                                                    //$scope.SaleBestData = data.max;
                                                var goods = data.latest_prod.items;                                                
                                                for(var i=0; i< goods.length && i < 6 ; i++){
                                                    if( i < 3 ){
                                                        $scope.buyTogether1.push(goods[i]);
                                                    }else{
                                                        $scope.buyTogether2.push(goods[i]);
                                                    }
                                                }
                                            })
                                            .error(function() {
                                ...
                                , error: function(data, status, err) {
                                     console.log('Data Error : func_buyTogether 로드 실패');
                                }
                        });
                };                
                if($scope.BasicData.chanel_yn == false){	
					...
					$scope.func_SaleBestData = function(callback) {
							...
							$.ajax({
									...
									, success: function(data) {
											...
											if(data.results != null && data.results.length > 0){
	                                            ...
	                                            .success(function(data) {	                                                    
	                                                    $scope.SaleBestData = data.max;
                                                        SaleBestDataCopy = $scope.SaleBestData.buybest.items;
	                                            })
	                                            .error(function() {
	                                                     console.log('Data Error : saleBestData로드 실패');
	                                            });                                            
				...
				$scope.reqDetailParam.sort = "L"; // 최초 사진상품평순 P 201701
				// 별점갯수
                $scope.c_star = {
                    v0 : true,v5:false,v4:false,v3:false,v2:false,v1:false                    
                }
                $scope.c_star_title = "별점전체";
				$scope.getCommentViewListPaging = function(pageNum,fun_gubun,searchVal) {
						pageNum *= 1;
						// 사진상품평순P/최근상품평순L 201701
						if(fun_gubun == "P" || fun_gubun == "L" || fun_gubun == "G"||
                          fun_gubun == "B" || fun_gubun == "R" || fun_gubun == "M"){  
				            $scope.reqDetailParam.sort = fun_gubun;
						}else if(fun_gubun == 0){ //별점 전체를 선택한 경우
                            $scope.c_star.v0 = true;
                            $scope.reqDetailParam.star = "0";
                            $scope.c_star_title = "별점 전체";
                            $scope.c_star.v5=false;
                            $scope.c_star.v4=false;
                            $scope.c_star.v3=false;
                            $scope.c_star.v2=false;
                            $scope.c_star.v1=false;
                        }else if(fun_gubun == 1 ||fun_gubun == 2 ||
                                fun_gubun == 3 || fun_gubun == 4 ||fun_gubun == 5){                            
                            $scope.c_star.v0=false;
                            var tmp_str = "", tmp_count = 0, tmp_str2 = "별점 ";                            
                            if($scope.c_star.v1){
                                tmp_str += "1_";
                                tmp_str2 += "1점+";
                                tmp_count ++;
                            ...
                            if($scope.c_star.v5){
                                ...
                                tmp_str2 = tmp_str2.substr(0, tmp_str2.length - 1);
                            }                
                            if(tmp_count == 0){ //한개도 선택 안했으면
                                tmp_str = "0";
                                tmp_str2 = "별점 전체";
                                $scope.c_star.v0 = true;
                            }
                            //console.log(tmp_str);
                            $scope.c_star_title = tmp_str2;
                            $scope.reqDetailParam.star = tmp_str;
                        }
						if(pageNum == 0 ){
								$scope.CommentViewList = [];
						}
						...
						/* 201701 상품평*/
                        if(fun_gubun == "S"){
                            $scope.reqDetailParam.goods_no = searchVal;
						};
                        if(fun_gubun == "S2"){
                            $scope.sort_tab = true;
                            $scope.sort_tab_open = false;
						...
						.success(function(data) {
								try {
										...
										}else if(data.data_set.stat == null){
                                            //201701 상품평개선    
                                            data.data_set.stat = $scope.nullprObj();                            
                                        }
								        $scope.PrdCommentData = data.data_set; //상품평
				...
				$scope.goQuestWrite = function(){
						...
						if($scope.BasicData.product.goods_cmps_cd == 30 && $scope.reqDetailParam.goods_no ==  $scope.BasicData.goods_no){
                            ...
                            if($scope.select_product){
                                LotteStorage.setSessionStorage("qnaindex", $scope.select_product.index - 1);
                            }
		...
		app.directive('productInfo', ['Fnproductview', '$window', '$http', 'LotteCommon',
				...
				return {
						...
						link : function($scope,el,attr){
								...
								/* 샤넬 하단 상품상세 이동 */
								$scope.ProviewGo = function (items, tclick, index) {
									if(index!=null){
										index = index + 1;
										if(index < 10){
									...
									var url = LotteCommon.prdviewUrl + "?" + $scope.baseParam + "&goods_no=" + items.goods_no;
		...
		// Directive :: 상품정보
		app.directive('productInfoDetail', ['Fnproductview', '$window', function(Fnproductview, $window) {
				return {
						...
						link : function($scope, el, attrs){
								...
								$scope.productLayer = function(goods_no, index) {
									...
									$scope.dimmedOpen({target : "productLayer"});

									$scope.swipeVideoStop(); // v-com : 동영상 정지
									// v-com : 상품상세 기획전 묶음상품 레이어 팝업 영역 - 동영상 초기화
									if($scope.productDetailLayer.mpic_url_addr != undefined && $scope.productDetailLayer.mpic_url_addr != ""){
										// 동영상이 있으면 초기화
										setTimeout(function(){
											autoVideoPlay('autoVideo2', '#autoVideo2');
										}, 100);
									};
									...
									//2016.12.19
									if($scope.pushstateIdx <= 1){
										history.pushState('addState',null);									
									}
								};				
								...
								$scope.closeProductDetailLayer = function() {
										...
										//$scope.isShowproductDetailLayer = false;
										//$scope.dimmedClose();
										$scope.pushstateIdx = 0;
										history.back();
								};
								//popstate  2016.12.19 이벤트 발생시
				...
				return {
						...
						link : function($scope, el, attrs){
								...
								angular.element($window).on('scroll', function(evt) {
									...
									if (this.pageYOffset >= $scope.detailTopHeight + headerTop){ // 45 = header
										$scope.tabFixedChange(true);	
										el[0].style.cssText = 'z-index:120;position:fixed;top:'+headerTop+'px;width:100%;';
									}else{
