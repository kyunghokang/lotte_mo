	...
	function changeMyDlvp(deli_cnt, gbn) {
		...
		// 내 배송지 수정취소
			...
			//alert($("#deli_mine input[name=deli_selected_no]").val());
			/**
			if($("#deli_mine input[name=tmp_radio_yn]").val()=="y"){
				$("#deli_mine input[name=deli_selected_no]").val($("#deli_mine input[name=tmp_deli_selected_no]").val());
			}**/
			//alert($("#deli_mine input[name=tmp_deli_selected_no]").val());
			$("#deli_mine input[name=deli_selected_no]").val($("#deli_mine input[name=tmp_deli_selected_no]").val());
	...
	function init_point() {
		...
		$("#chk_b2e_point").attr("checked", false); // B2E-포인트
		...
		$("#chk_b2e_point").prop("checked", false); // B2E-포인트
	...
	function select_third_coupon() {
		...
		if ($("#frm_send input[name=prommdclcd_third]").val() == "35"){ // 일시불인 경우			
			...
			$("#chk_b2e_point").prop("checked", false);
			$("#chk_b2e_point").prop("disabled", true);
			...
			$("#chk_b2e_point").prop("disabled", false); // B2E포인트 제어 2016.03.15 최창표
		}
	...
	function fn_selPointAll(obj) {
		...
		if (obj.checked){
			...
			}else if (obj_id == 'chk_b2e_point_all'){// B2E_포인트
				totsttlamt = parseInt(totsttlamt) + parseInt($("#old_b2e_point_amt").val());
				useable_amt = parseInt($("#useable_b2e_point_amt").val());
					
				$("#b2e_point_amt").val((totsttlamt<useable_amt?totsttlamt:useable_amt));
			...
			}else if (obj_id == 'chk_b2e_point_all'){// B2E 포인트
				$("#b2e_point_amt").val("");
				$("#old_b2e_point_amt").val("0");
	...
	function fn_point_change(obj){
		...
		} else if (obj_id == "b2e_point_amt" && $("#chk_b2e_point").prop("checked")){ // B2E_포인트
			useable_amt = parseInt($("#useable_b2e_point_amt").val());
			$("#chk_b2e_point_all").prop("checked", false);
	...
	function send() {
		...
		if( "Y" != grockle_yn ){
			...
			if (sendSms=='Y' && (gubun=='SMARTPICK' || gubun=='ECOUPON') ){
				...
				//마스터 휴대폰번호로 우선 셋팅
			    $("#frm_send input[name=mst_adre_nm]:hidden").val($("#frm_inp input[name=mst_adre_nm_1]").val());
			    $("#frm_send input[name=mst_adre_no_01]:hidden").val($("#frm_inp select[name=mst_adre_no_01_1]").val());
			    $("#frm_send input[name=mst_adre_no_02]:hidden").val($("#frm_inp input[name=mst_adre_no_02_1]").val());
			    $("#frm_send input[name=mst_adre_no_03]:hidden").val($("#frm_inp input[name=mst_adre_no_03_1]").val());			    
				...
				$("#frm_send input[name=msg_cont]:hidden").val($("#frm_inp textarea[name=msg_cont_1]").val());
		...
		// 포인트정보
		var b2e_point_amt, lpoint_amt, lt_point_amt, deposit_amt, soil_point_amt, chk_point, chk_point_all, pay_mean_cd, pay_amt;
		b2e_point_amt   = ($("#b2e_point_amt").prop("id")==undefined?"":(($("#b2e_point_amt").val()).length<1?0:$("#b2e_point_amt").val())); // 2016.02.19 B2E_포인트
		...
		$.each($("#frm_inp input:checkbox[name=chk_point]:checked"), function(){
				...
				if($(this).val()=="b2e_point"){ // B2E_포인트
					pay_cd_arr.push(b2ePoint);
					pay_amt_arr.push(b2e_point_amt);
		...
		$("#frm_send input[name=b2e_point_amt]:hidden").val( b2e_point_amt );		
		...
		if (paytype == PAYTYPE_CODE_BANK || (lpoint_amt != "" && lpoint_amt > 0) || (b2e_point_amt != "" && b2e_point_amt > 0) || (deposit_amt != "" && deposit_amt > 0)){
			...
			if (cash_cd == "1"){ // 소득공제
				...
				if (cash_val == null || cash_val == ''){ // 전화번호첫자리 alert 추가 2016.02.21 최창표
					alert("전화번호첫자리를 선택해 주세요.");
					$("#frm_inp select[name=cr_issu_mean_no_phone1]").focus();
					bool_send = false;
					return false;
				}
			  ...
			  || X_CARDTYPE == paytype_card_018
				...
				xmpi_exec(); // xmpi 실행
			      ...
			      && (cp_udid==''
					  ...
					  || cp_schema=='mlotte001B2B'
				      ) 
				  ){ // B2B모바일 앱 관련 추가 2016.03.11 최창표
				payment_pop_position(); // 결제 레이어팝업 위치 지정
	...
	function fn_selPoint(obj) {
		...
		if (obj.checked){
			...
			}else if (obj_id=='chk_b2e_point'){ // B2E_포인트
				useable_amt = parseInt($("#useable_b2e_point_amt").val());
				//useable_amt = (Math.floor(useable_amt/10)) * 10; // 10원이하 절사
				
				$("#b2e_point_amt").val((totsttlamt<useable_amt?totsttlamt:useable_amt));
				$("#b2e_point_amt").attr("disabled", false);
				$("#chk_b2e_point_all").attr("disabled", false);
				$("#chk_b2e_point_all").attr("checked", true);
				$("#chk_b2e_point_all").prop("checked", true);
				//sendTclick('order_Clk_Btn_2');
	...
	function fn_calcTotalPrice(){
		...
		b2e_point_amt   = ($("#b2e_point_amt").prop("id")==undefined?"":(($("#b2e_point_amt").val()).length<1?0:$("#b2e_point_amt").val())); // B2E_포인트
		...
		// B2E_포인트 사용 시
		if (b2e_point_amt > 0){
			totpayamt   = (totpayamt - b2e_point_amt).toString();
		}
		...
		if ( third_cpn_amt > 0 ){
			totalPriceHtml = apply_br_tag(totalPriceHtml);
			if(isB2B) {
				totalPriceHtml += '<li class="pr_text1">'+(prommdclcd_third=="35"?'특별추가할인':'')+'</li>'
		...
		if ( b2e_point_amt > 0 ) {	
			...
			totalPriceHtml += '<li class="pr_text1">포인트</li>'
					        + '<li class="pr_text2">(-)' + b2e_point_amt.money() + '점</li>';

			$("#old_b2e_point_amt").val(b2e_point_amt);

			discountSum += parseInt(b2e_point_amt);
			pointSum += parseInt(b2e_point_amt);
		} else if ( b2e_point_amt > -1 ) {
			$("#old_b2e_point_amt").val(b2e_point_amt);
		}
...
//			discountSum += parseInt(dlv_fvr_val);
		...
		if (totpayamt == 0)
		{
			...
			// b2e포인트 현금영수증관련 처리 2016.02.21 최창표
			if ( $("#frm_inp input[name=rdo_paytype]:radio:checked").val() == paytype_code_bank ) {	
				$("#cash_receipts").hide();
			}
			// b2e포인트 전액 결제시 현금영수증 화면 처리 2016.02.22 최창표
			$("#frm_inp input[name=rdo_cash]:radio").each(function(rdo_idx) {
				$(this).attr("checked",(rdo_idx == 2));
				$(this).prop("checked",(rdo_idx == 2));
			});
			$("#frm_inp input[name=rdo_cash]:radio:checked").click(); // 현금영수증 영역 선택 event 발생
		}
		else
		{
			...
			if ( $("#frm_inp input[name=rdo_paytype]:radio:checked").val() == paytype_code_card || $("#frm_inp input[name=rdo_paytype]:radio:checked").val() == paytype_code_lpay ) {
				...
				$("#cash_receipts").show(); // b2e포인트 현금영수증관련 처리 2016.02.21 최창표
				// b2e포인트 부분 결제시 현금영수증 화면 처리 2016.02.22 최창표
				$("#frm_inp input[name=rdo_cash]:radio").each(function(rdo_idx) {
					$(this).attr("checked",(rdo_idx == 0));
					$(this).prop("checked",(rdo_idx == 0));
				});
				$("#frm_inp input[name=rdo_cash]:radio:checked").click(); // 현금영수증 영역 선택 event 발생
			}
	...
	function fn_initPointList(point_div){
		...
		if (point_div == "ALL" || point_div == b2ePoint){
			$("#b2e_point_amt").val("");
			$("#old_b2e_point_amt").val("");
			$("#b2e_point_amt").attr("disabled", true);
			$("#chk_b2e_point_all").attr("checked", false);
			$("#chk_b2e_point_all").prop("checked", false);
			$("#chk_b2e_point_all").attr("disabled", true);
		}
	...
	function setCashReceiptArea(){
		...
		var chk_b2e_point = $("#chk_b2e_point").prop("checked"); // B2E포인트 사용 2016.03.11 최창표
		...
		if (pay_type != PAYTYPE_CODE_BANK && !chk_lpoint && !chk_deposit_all && !chk_b2e_point){  // B2E포인트 사용 추가 2016.03.11 최창표
	...
	$(window).load(function(){
		...
		$("#chk_multi_dlvp").click(function() {
			...
			if( $("#chk_multi_dlvp").prop("checked") ){
				...
				if(deli_size > 0){ // 내배송지가 있을때만 처리 2016.02.21 최창표
					changeMyDlvp('1', 'cncl');
				}
				$("#changeMyDlvp").hide();
				$("#changeNameM").show();
		...
		$("#frm_inp select[name=dup_coupon]").click(function() {
			...
			if( "27" == $("#frm_send input[name=prommdclcd]").val() ){
				if(isB2B) {
					alert("임직원할인 적용 시 추가할인 및 특별추가할인 혜택을 받으실 수 없습니다.");
				} else {
	   ...
	   // var meg = ' 님께서 보내시는 선물입니다.';
		...
		// B2E포인트 기본 선택 B2B모바일 2016.03.09 최창표
		if ( !$("#frm_inp #chk_b2e_point").attr('disabled') ) {
			$("#frm_inp #chk_b2e_point").trigger('click');
		}
		
		// lpay setting
		if (typeof lpayment != "undefined") { // 20160726 박형윤 lpay 접속지연으로 인한 주문서 스크립트 오류 방지 추가
			lpayment.windowLoad();
		}
	});
