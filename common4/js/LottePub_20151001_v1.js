...
var scrollHandler = function (e) {
    ...
    if ( tag.mainNavOne && tag.mainNavTwo ) {
        ...
        if ( document.querySelectorAll( 'div.reated_keyword' ).length != 0 ) {
            ...
            if ( opt.scrollTop >= opt.mainHeaderHeight ) {
                tag.mainHeader.style.marginBottom = ( opt.subHeaderHeight + opt.reated_keywordHeight + opt.srh_terms_wrapHeight ) + 'px' ;
...
function appIntroFunc () {
    ...
    if ( !getCookieValue( 'firstAppNoti' ) &&
            ((uagent.indexOf("android") > -1 && (uagent.search("2.24") > -1 || uagent.search("1.08") > -1)) // android
            || (uagent.indexOf("android") < 0 && (uagent.search("2.15") > -1 || uagent.search("2.14") > -1 || uagent.search("1.34") > -1 || uagent.search("1.35") > -1))) ){ // IOS
    ...
    for ( t in transitions ) {
        if ( el.style[t] !== undefined ) {
            ...
            if ( ua.match( 'Android' ) != null ) {
                ...
                var galaxyS3 = device_android.match( 'SHV-E210' ) != null ? true : false ; /* KCC-CMM-SEC-SHVE210 */
