 ...
 * 검색관련 : S
 ***********************************/
var defaultSearchLayerTabIdx = 0;
var doSearchRenew = function () {
	
	//var searchURL = __defaultDomain + "/search/m/search_keyword.do";		// WEB 
	var searchURL = __defaultDomain + "/search/m/search_keyword_app.do";	// app과 동일하게 json 형식 변경 2015.10.01
	...
	openSrhLayerBody = function(tg){ /*native call*/		
		if(isApp && !isIpad){ /* App인 경우*/
			...
			showBestKeyword(); 			//2015.10.01 인기검색어,급상승검색어 함수추가 
			$("#cover").addClass("nsh"); //2015.10.01 추가
	        $("#container").hide(); 	 //2015.10.01  추가
			/* 최근 검색어 보기 */
			showRecentKeyword();
			/* IScroll 새로고침 */
			//refreshIScroll();	2015.10.01 삭제
			/* cover update */
			//coverUpdate();			
			 ...
			 if(response.result.length > 0){ 	
				setTimeout(function(){ // 시간차가 없으면 휴대폰에서 정상동작안함
					...
					if($.trim(_$searchKeyword.val()).length == 0){
					 ...
					 var $items = response.result[0].items,
	                     tmpStr = "",
	                     paramKeyword = "";
                 
	                 //20150930 수정
	                 for(var i = 0;i<response.result[0].totalcount;i++){
	                     paramKeyword = $items[i].hkeyword.replace(/<[a-z/]+>/gi,"");
	                     if(i>0){
	                         if($items[i].hkeyword != $items[i-1].hkeyword){
	                             tmpStr += "<li><a href=\"javascript:fn_goSearch('"+paramKeyword+"', '&amp;reqType=N&amp;tclick=M_Search_Autocomplete_Web');\" class=\"kword\">"+$items[i].hkeyword+"</a><div onClick=\"searchSet('"+paramKeyword+"');\" class='arrowkey'></div></li>";
	                         }
	                     }else{
	                        tmpStr += "<li><a href=\"javascript:fn_goSearch('"+paramKeyword+"', '&amp;reqType=N&amp;tclick=M_Search_Autocomplete_Web');\" class=\"kword\">"+$items[i].hkeyword+"</a><div onClick=\"searchSet('"+paramKeyword+"');\" class='arrowkey'></div></li>";
	                     }
	                     if($items[i].linkurl != ""){
	                         tmpStr += "<li><a href=\"javascript:fn_goSearch('"+paramKeyword+"', '&reqType=N&tclick=M_Search_Autocomplete_Web&cateNo="+ $items[i].linkurl +"&reqType=N&tclick=M_Search_Autocomplete_Web');\" class=\"categoryWord\">"+$items[i].linkname+"</a></li>";
	                     }
	                 }	                    
	                 
	                 $(".srh_auto > ol").html(tmpStr);
	
	                 _$auto.css('display','block');
	                 $(".nsh_tabs").css('display','none');
	                 _$recentDel.css('display','none');
	
	                 if($.trim(_$searchKeyword.val()).length == 0){
	                     _$auto.css('display','none');
	                     $(".nsh_tabs").css('display','block');
	                 }						
				},500);
			 }
			}
	...
	closeSrhLayor = function(){
		...
		//_$searchKeywordDel.hide();
		nsh_tabSelect(defaultSearchLayerTabIdx);
		$("#container").show(); 
        $("#cover").removeClass("nsh");
	...
	function init(){
		/* IScroll 셋팅 */
		//setIScroll();		2015.10.01 삭제
		/* cover update */
		//coverUpdate();	2015.10.01 삭제
	}
	...
	function mainSearchHandler(){
		...
		// App-WEB인 경우
			...
			//refreshIScroll(); // IScroll 새로고침			2015.10.01 제거
			//coverUpdate(); // cover update				2015.10.01 제거
	...
	function showRecentKeyword(){
		...
		$(".nsh_tabs").css('display','block'); //eung 추가
		...
		if (!recentKeyList || recentKeyArray.length == 0) {
			defaultSearchLayerTabIdx = 1;
		} else {
			defaultSearchLayerTabIdx = 0;
		}

        nsh_tabSelect(defaultSearchLayerTabIdx);    //20150930-1    
        $(".barInfo").hide();//20150930-1 
		
		//myScroll1.options.click = true;		2015.10.01 제거
		//myScroll2.options.click = true;		2015.10.01 제거
		
		/* IScroll 새로고침 */
		//refreshIScroll();						2015.10.01 제거
	}
		...
		$.ajax({
			...
			, success: function(html) {
				...
				if($B.string.trim(html) == ''){							//결과가 없으면
					$('#brand_no_result').show().html('<strong>' + searchBText+ '</strong>(으)로 검색된 브랜드가 없습니다.');
				}else{
					$('#brand_no_result').hide();
...
var nsh_swipe;
//인기 & 급상승 검색어
function showBestKeyword(){
    var i=0,
        $items,
        tmpStr = "<ol>";        
    $.ajax({
         url: "/json/searchKeywordLayer.json" //localtest 용, json 실제 링크로 바꿔주세요.
         //url: "/search/searchLayer_0901_data.html" //localtest 용, json 실제 링크로 바꿔주세요.
        , async : true, cache : true, dataType : "json"
        , success: function(data) {
            //console.log("result:" + data.result_best.items);
    	    data = data.searchPopList;
            //인기검색어 생성
            $items = data.result_best.items;
            for(;i<$items.length;i++){
                tmpStr += "<li><a href=\"javascript:fn_goSearch('"+$items[i].hkeyword+"', '&amp;tclick=M_Search_RecentSearch_Web');\" class=\"word\"><span class=\"pre\">"+(i+1)+"</span>"+$items[i].hkeyword+"</a></li>";
            }
            //eung 0918 날짜 추가 20150930-1 삭제
            //tmpStr += "<li class='nsh_date'>"+data.date+" 기준</li>";
            
            tmpStr += "</ol>";
            $("#nsh_best").html(tmpStr);
            //급상승검색어 생성, 0 제자리,1 up, 2 down, 3 new
            tmpStr = "<ol>";
            $items = data.result_speed.items;
            for(i=0;i<$items.length;i++){                    
                tmpStr += "<li><a href=\"javascript:fn_goSearch('"+$items[i].hkeyword+"', '&amp;tclick=M_Search_RecentSearch_Web');\" class=\"word\"><span class=\"pre\">"+(i+1)+"</span>"+$items[i].hkeyword;
                switch($items[i].rate_type){
                    case 0:
                        tmpStr += "</a></li>";
                        break;
                    case 1:
                        tmpStr += "<span class=\"lank up\">"+$items[i].rate+"</span></a></li>";
                        break;
                    case 2:
                        tmpStr += "<span class=\"lank down\">"+$items[i].rate+"</span></a></li>";
                        break;
                    case 3:
                        tmpStr += "<span class=\"lank\">new</span></a></li>";
                        break;
                }                    
            }
            //eung 0918 날짜 추가 20150930-1 삭제
            //tmpStr += "<li class='nsh_date'>"+data.date+" 기준</li>";
            $(".barInfo > .dateInfo").html(data.date+" 기준");
            
            tmpStr += "</ol>";
            $("#nsh_speed").html(tmpStr);
            //스와이핑 등록
            //eung 0918
            setTimeout(function(){
                nsh_swipe =  setEGSlide("#nsh_box", function(index){  
                    nsh_setTab(index);
                }, false);
            ...
            console.log("인기&급상승검색어 데이타 오류");
        }
    });        
}
//탭선택표시 20150930-1
function nsh_setTab(index){
    $(".nsh_tab_a").removeClass("on").eq(index).addClass("on");
	viewAutolayer(false);
    if(localStorage.getItem('myWord') == '' || index > 0){
		$(".srh_bar > .del").hide();
        $(".barInfo").show();
	}else{            
        $(".srh_bar > .del").show();
...
//20150930-1 자동검색 하단
function viewAutolayer(flag){
    $(".barInfo > div").hide();
    if(flag){//자동검색이면 
        $(".del").hide();
        $(".barInfo > .bestInfo").show();
        $(".barInfo").show();
    }else{
        $(".barInfo > .dateInfo").show();
    }                
}
//탭이동 20150924
function nsh_tabSelect(index){
    if(nsh_swipe != undefined){
        nsh_swipe.setMoveX(-index*parseInt($(".nsh_mask").width()), 300, function(){
            nsh_setTab(index);    
        });
    }
}    
